...
const block=[
 ...
 "  await tap(page.locator('#v8Sets [data-cnt=\"1\"]'));await tap(page.locator('#v8Sets [data-cnt=\"1\"]'));await tap(page.locator('#v8Sets [data-cnt=\"1\"]'));await page.waitForFunction(()=>document.querySelectorAll('#v8Sets [data-si]').length===4,undefined,{timeout:1500});await tap(page.locator('#v8Sets [data-si=\"0\"]'));const fullSetValues=async()=>page.locator('#v8Sets [data-si=\"0\"] span b').evaluateAll(xs=>xs.slice(0,2).map(x=>Number(x.textContent)));const fullBeforeSetValues=await fullSetValues();assert.equal(fullBeforeSetValues.length,2,'full-family first set values missing before physical edit');const fullWeightPlus=page.locator('#axisSetControls [data-axis-step=\"weight\"][data-dir=\"1\"]');assert.equal(await fullWeightPlus.count(),1,'full-family visible set-plan weight stepper missing');assert.equal(await fullWeightPlus.isVisible(),true,'full-family set-plan weight stepper is not visible');await tap(fullWeightPlus);const fullRepsPlus=page.locator('#axisSetControls [data-axis-step=\"reps\"][data-dir=\"1\"]');assert.equal(await fullRepsPlus.count(),1,'full-family visible set-plan reps stepper missing');assert.equal(await fullRepsPlus.isVisible(),true,'full-family set-plan reps stepper is not visible');await tap(fullRepsPlus);await tap(fullRepsPlus);await page.waitForFunction(before=>{const xs=[...document.querySelectorAll('#v8Sets [data-si=\"0\"] span b')].slice(0,2).map(x=>Number(x.textContent));return xs.length===2&&xs[0]>Number(before[0])&&xs[1]===Number(before[1])+2},fullBeforeSetValues,{timeout:1800});const fullEditedSetValues=await fullSetValues();assert.ok(fullEditedSetValues[0]>fullBeforeSetValues[0],'full-family visible Group Plan weight control did not change first set');assert.equal(fullEditedSetValues[1],fullBeforeSetValues[1]+2,'full-family visible Group Plan reps control did not change first set twice');",
 ...
 "  const fullEvent=await savedEvent(full.id);assert.deepEqual(fullEvent.schema,['weight','reps','sets','hold','distance','pace','incline','completed']);assert.equal(fullEvent.metrics.weight,fullEditedSetValues[0]);assert.equal(fullEvent.metrics.reps,fullEditedSetValues[1]);assert.equal(fullEvent.metrics.sets,4);assert.equal(fullEvent.legacy.weight,fullEditedSetValues[0]);assert.equal(fullEvent.legacy.reps,fullEditedSetValues[1]);assert.equal(fullEvent.legacy.sets,4);assert.equal(fullEvent.metrics.hold,45);assert.equal(fullEvent.metrics.distance,3);assert.equal(fullEvent.metrics.pace,'5:30');assert.equal(fullEvent.metrics.incline,6.5);assert.equal(fullEvent.metrics.completed,false);"
].join('\n');
