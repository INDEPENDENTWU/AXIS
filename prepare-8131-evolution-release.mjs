...

{
 ...
 const cloudFrom="const cloudHead=await page.locator('#v811ServicePanel .v813ServiceHead span').first().evaluate(el=>getComputedStyle(el).fontSize);\n assert.equal(cloudHead,await page.locator('#settingsSheet .settingPlain>span').first().evaluate(el=>getComputedStyle(el).fontSize));";
 const cloudTo="const cloudHead=px(await page.locator('#v811ServicePanel .v813ServiceHead span').first().evaluate(el=>getComputedStyle(el).fontSize));\n assert.ok(cloudHead>=11.5&&cloudHead<=16,`Cloud/AI heading typography drifted: ${cloudHead}`);";
