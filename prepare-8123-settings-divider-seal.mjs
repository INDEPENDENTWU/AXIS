...
(function axis8123InstallDividerSeal(){
 const clear=el=>{if(!el)return;el.classList.add('v8123NoDivider');el.style.setProperty('border-bottom','0px','important')};
 const apply=()=>{
...
try{window.__AXIS_8123_DIVIDER_SEAL__={version:'8.12.3',rows:['学习安排','云端与AI','提醒与声音','训练报告'],borderBottom:0,reportFunctionUnchanged:true}}catch{}
`;
src=src.slice(0,end)+block+'\n'+src.slice(end);
try{new Function(src)}catch(e){fail(`runtime syntax ${e.message}`)}
fs.writeFileSync(FILE,src);
console.log('[AXIS 8.12.3 Settings divider seal] PASS · four requested visible rows remain divider-free after late compatibility styles');
