...
}
{
  ...
  wm=regexOnce(wm,/function suppressLegacy\(\)\{[\s\S]*?\}\nfunction bind\(\)\{/,"function suppressLegacy(){if(!armed)return;const a=armed;armed=null;setTimeout(()=>finishStamp(a),0)}\nfunction bind(){",'8.8.4 temporary raw-mode suppression retirement');
  const end=wm.lastIndexOf('})();');if(end<0)fail('watermark runtime IIFE end missing');
...
await import('./prepare-818-object-focus-driver.mjs');
