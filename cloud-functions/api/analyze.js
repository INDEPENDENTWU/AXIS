...
const rate=globalThis.__axisEdgeVisionRate||(globalThis.__axisEdgeVisionRate=new Map());
...
export async function onRequestGet(context){
  ...
  return json({...publicAIConfig(c),model:c.visionModel});
}

export async function onRequestPost(context){
  const c=getAIConfig(context.env||{});
  if(!c.enabled||!c.visionEnabled)return json({available:false,error:'not_available'},503);
  const ip=context.clientIp||context.request.headers.get('x-forwarded-for')||'unknown';
  if(!allow(String(ip).split(',')[0].trim(),c.visionRPM))return json({available:true,error:'rate_limited'},429);
  ...
  if(!frames.length)return json({available:true,error:'no_frames'},400);
  if(frames.reduce((sum,x)=>sum+String(x||'').length,0)>c.maxImageChars)return json({available:true,error:'too_large'},413);
  if(frames.some(x=>!/^data:image\/(jpeg|jpg|png|webp);base64,/i.test(String(x))))return json({available:true,error:'bad_frame'},400);
  ...
  try{
    ...
    if(shouldEscalate){try{const second=await callModel(c,c.visionFallbackModel,content);if(second.confidence>result.confidence){result=second;escalated=true}}catch{}}
    return json({available:true,result:{...result,escalated}});
  }catch(e){return json({available:true,error:e?.name==='AbortError'?'timeout':e?.message||'network'},502)}
}
