...
export async function onRequestPost(context){
 const c=getAIConfig(context.env||{});
 if(!c.enabled||!c.visionEnabled||!c.visionProviders.length)return json({available:false,error:'not_available'},503);
 const ip=String(context.clientIp||context.request.headers.get('x-forwarded-for')||'unknown').split(',')[0].trim();
 if(!allow(ip,c.visionRPM))return json({available:true,error:'rate_limited'},429);
 ...
 try{
  ...
  return json({available:true,error:e?.name==='AbortError'?'timeout':e?.message||'network'},502);
 }
}
