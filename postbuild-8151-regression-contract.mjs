...
const contract=JSON.parse(read('release-contract.json')),info=JSON.parse(read('axis-build.json')),runtime=read('axis-core.js'),css=read('axis-style.css'),app=read('app.js'),wm=read('v8710-watermark.js');
...
info.axis8151={release:true,scope:'coldstart-watermark-regression-seal',home:{staticHero:'geometry-only-before-canonical-render',readyOwner:'app.js renderHomeState',historicalSemanticFlash:false},watermark:{photoOwner:'v8710-watermark',legacyAppPhotoCompositor:false,centerBrand:false,currentCard:true,videoOwner:'inherited-unchanged'},ownership:{newPersistence:false,newNetwork:false,trainingState:false,evolution:false,replay:false}};
fs.writeFileSync('axis-build.json',JSON.stringify(info,null,2)+'\n');
console.log('[AXIS 8.15.1 regression contract] PASS · canonical Home first paint · single saved-photo watermark card · no legacy lower-left or center brand');
