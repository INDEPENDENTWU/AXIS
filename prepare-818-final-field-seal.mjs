   ...
   tap must wait for that exact camera acquisition instead of returning false. */
{
 ...
 const legacyScanClickSingle="$('#scanSeconds button').forEach(b=>b.onclick=()=>{state.prefs.scanSeconds=Number(b.dataset.sec);save();renderSettings()});";
 const legacyScanClickDouble="$$('#scanSeconds button').forEach(b=>b.onclick=()=>{state.prefs.scanSeconds=Number(b.dataset.sec);save();renderSettings()});";
 const legacyScanClickSingleCount=s.split(legacyScanClickSingle).length-1,legacyScanClickDoubleCount=s.split(legacyScanClickDouble).length-1;
 if(legacyScanClickSingleCount+legacyScanClickDoubleCount!==1)fail(`legacy scan click writer expected one compiler shape, found ${legacyScanClickSingleCount}/${legacyScanClickDoubleCount}`);
 s=s.replace(legacyScanClickSingleCount?legacyScanClickSingle:legacyScanClickDouble,'');
