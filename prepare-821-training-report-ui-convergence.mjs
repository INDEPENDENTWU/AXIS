 ...
 * than requiring the intentionally retired v8710 three-card/JPG projection. */
{
  ...
  const current=`const reportFixture=await page.evaluate(()=>{try{const x=JSON.parse(localStorage.getItem('axis_v60_state')||'{}');return{sessions:Array.isArray(x.sessions)?x.sessions.length:0,active:!!x.active}}catch{return{sessions:0,active:false}}});
assert.equal(reportFixture.sessions,0,'Chromium Runtime fixture unexpectedly contains completed Sessions');
assert.equal(reportFixture.active,true,'Chromium Runtime fixture lost its in-progress Session before Report truth check');
...
assert.equal((await page.locator('#axis821ReportScope').innerText()).trim(),'全部完成记录','Settings report must project completed Sessions only');
await page.waitForSelector('#reportPreview .axis821ReportEmpty',{state:'visible',timeout:1200});
assert.match((await page.locator('#reportPreview .axis821ReportEmpty').innerText()).trim(),/暂无可用训练记录/,'Chromium completed-history empty state is missing');
assert.equal(await page.locator('#reportPreview .axis821ReportHero').count(),0,'Chromium active Session leaked into completed-history aggregates');
...
const reportOwner=await page.evaluate(()=>window.__AXIS_821_TRAINING_REPORT_UI__);assert.equal(reportOwner?.truthSchema,'axis.report-range.v1');assert.equal(reportOwner?.reportUIOwner,true);assert.equal(reportOwner?.exportOwner,false);assert.equal(reportOwner?.legacyShareExport,false);`;
  s=replaceSpan(s,start,end,current,'Chromium Runtime completed-history Report contract');
