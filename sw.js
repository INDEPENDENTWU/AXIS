...
self.addEventListener('fetch',e=>{
  ...
  if(fresh){
    e.respondWith(fetch(e.request).then(r=>{if(r.ok){const x=r.clone();caches.open(CACHE).then(c=>c.put(e.request.mode==='navigate'?'/index.html':e.request,x))}return r}).catch(()=>caches.match(e.request.mode==='navigate'?'/index.html':e.request)));
    return;
  }
