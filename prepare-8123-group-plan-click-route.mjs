 ...
 * owner. The old delegated set-bridge listener remains available for legacy
 * launchers, but this canonical button stops propagation after routing through
 * the same existing planner APIs. This removes duplicate open/upgrade chains
 * without taking over plan state or calculations.
 */
{
...
`;
 ...
 src=src.slice(0,end)+"try{window.__AXIS_8123_GROUP_PLAN_CLICK_ROUTE__={version:'8.12.3',launcherOwner:'v61',plannerOwner:'v874-set-bridge',surfaceOwner:'v8712-runtime',loadOrderSafe:true,retryWindowMs:2000,propagationStopped:true,scope:'canonical-launcher-only'}}catch{}\n"+src.slice(end);
 for(const needle of ['__AXIS_8123_GROUP_PLAN_CLICK_ROUTE__','__AXIS_GROUP_PLAN_OPEN__','__AXIS_GROUP_PLAN_UPGRADE__','axis8123OpenGroupPlan()','e.stopImmediatePropagation()','propagationStopped:true'])if(!src.includes(needle))fail(`missing ${needle}`);
 try{new Function(src)}catch(e){fail(`v61 syntax ${e.message}`)}
 fs.writeFileSync(FILE,src)
}
console.log('[AXIS 8.12.3 Group Plan click route] PASS · canonical launcher has one v61 click owner; existing set-bridge planner and v8712 surface remain unchanged');
