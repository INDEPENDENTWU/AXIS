...
const finalStageCss=read('styles/axis-821-active-home.css');
...
for(const marker of ['.v87Rest{box-sizing:border-box!important;height:auto!important;min-height:34px!important;margin:12px auto 0!important','display:flex!important','background:rgba(115,124,255,.07)!important','animation:axis826RestStateIn .24s','.v87Rest{animation:none!important}'])if(!finalStageCss.includes(marker))fail(`final Active stage rest binding missing ${marker}`);
...
for(const source of [css,finalStageCss])for(const forbidden of ['localStorage.setItem','sessionStorage.setItem','indexedDB.open','state.active.events.push(','writeCore(','writeMeta(','fetch(','XMLHttpRequest','WebSocket('])if(source.includes(forbidden))fail(`presentation CSS acquired forbidden authority ${forbidden}`);
info.gates=info.gates||{};
Object.assign(info.gates,{activeRestSelectorBinding8262:true,activeRestCanonicalNode8262:true,activeRestPresentationOnly8262:true,activeRestReducedMotion8262:true,activeRestFinalStageOwner8262:true});
info.axis8262={release:true,scope:'active-rest-selector-binding',finding:'8.26.1 presentation selector did not match canonical v87Rest node',presentation:{canonicalSelector:'.v87Rest',restSpacing:true,restStatusPill:true,reducedMotion:true,finalStageOwner:'axis821ActiveStageRefineStyle'},ownership:{sessionWriter:false,encounterWriter:false,activeOwner:false,recorder:false,newPersistence:false,network:false,ai:false}};
fs.writeFileSync('axis-build.json',JSON.stringify(info,null,2)+'\n');
await import('./scripts/axis-8262-governance-compat.mjs');
console.log('[AXIS 8.26.2 Active Rest Selector contract] PASS · canonical .v87Rest binding is live in final Active stage owner · inherited Active truth preserved · no factual authority added');
