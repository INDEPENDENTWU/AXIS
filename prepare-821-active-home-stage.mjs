...
const finalVisualCss=read('styles/axis-821-active-home.css').trim();
if(!finalVisualCss.includes('background:transparent!important')||!finalVisualCss.includes('#v87AdjustBtn{position:static!important')||!finalVisualCss.includes('.axis821StageFinish:active'))fail('final visual source contract drift');
 ...
 * to-finish. Final flat/centered/tactile presentation is now source-owned here;
 * the historical late visual-convergence prepare is no longer a build owner.
 * The established postbuild Active-adjust hook is deliberately retained: the
 * stage keeps .v87Actions and the canonical render tail that postbuild patches.
 */
{
...
#v87Now.axis821ActiveStage #v87AdjustBtn{grid-column:1/-1;height:42px;border-radius:13px;background:transparent;color:var(--muted);font-size:11px;font-weight:600;border:1px solid rgba(255,255,255,.05)}
...
`;
 ...
 s=s.replace(boot,boot.replace('injectStyle();ensureUI();','injectStyle();axis821ActiveStageStyle();axis821ActiveStageRefineStyle();ensureUI();'));
 ...
 if(s.includes('__AXIS_821_ACTIVE_HOME_STAGE__')||s.includes('__AXIS_821_ACTIVE_HOME_VISUAL__'))fail('capability marker duplicated');
 s=s.slice(0,end)+marker+s.slice(end);
 syntax(s,FILE);write(FILE,s);
}

console.log('[AXIS 8.21 Active Home stage] PASS · ordinary Active is one integrated Home stage · final flat/centered/tactile visual source is owned here · one visible rest clock · v87 remains sole action owner · no new storage/writer/Active owner');
