...
/* v874: the same capability semantics drive editor inference and custom types. */
{
 ...
 s=replaceFunction(s,'function axis821EditorAutoMode()',`function axis821EditorAutoMode(){const xs=axis818MetricDraft||[],keys=new Set(xs.map(x=>String(x?.key||x?.id||'')).filter(Boolean)),hints=new Set(xs.map(x=>String(x?.extensions?.axis?.executionHint||AXIS821_EDITOR_EXECUTION_HINTS[String(x?.key||x?.id||'')]||(({duration:'timed',distance:'timed',pace:'timed',boolean:'complete'})[String(x?.type||'')]||'context'))));if(keys.has('rounds')||hints.has('rounds'))return'rounds';if(keys.has('sets')||(keys.has('weight')&&keys.has('reps'))||hints.has('sets'))return'sets';if(keys.has('hold')||hints.has('hold'))return'hold';if(keys.size===1&&keys.has('completed'))return'complete';if(hints.has('timed'))return'timed';if(xs.length===1&&hints.has('complete'))return'complete';return'single'}`,'editor capability execution resolver');
...
const app=read('app.js'),editor=read('v874-professional.js');
const singleOwnership="keys.has('sets')||(keys.has('weight')&&keys.has('reps'))||hints.has('sets')";
if(!app.includes(singleOwnership))fail('runtime execution resolver lost set-plan ownership boundary');
if(!editor.includes(singleOwnership))fail('editor execution resolver lost set-plan ownership boundary');
if(app.includes("keys.has('sets')||keys.has('reps')||hints.has('sets')")||editor.includes("keys.has('sets')||keys.has('reps')||hints.has('sets')"))fail('reps-only false sets inference survived capability convergence');
if((app.match(/state\.active\.events\.push\(/g)||[]).length!==1)fail('Encounter writer count drift');
...
console.log('[AXIS 8.21 Object capability convergence] PASS · 14 built-ins + 9 custom types · schema facts do not claim execution ownership · choice control · document-safe localization · no new factual owner');
