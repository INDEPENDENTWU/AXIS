...
`;
  ...
  const finalRender=functionRange(s,'function renderReport()','final Training Report renderer').text;
  const forbiddenProfile='state.'+'profile',forbiddenWeek="reportRange==='"+'7'+"'",forbiddenMonth="reportRange==='"+'30'+"'";
  for(const forbidden of [forbiddenProfile,forbiddenWeek,forbiddenMonth])if(finalRender.includes(forbidden))fail(`final renderer forbidden token ${forbidden}`);
  ...
  syntax(runtime+'\n'+render+'\nasync function makeReportImage(){return null}','isolated Training Report runtime');
