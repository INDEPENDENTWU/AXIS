...
| `prepare-821-flow-active-boot-scope.mjs` as a corrective private-listener relocation | retired from build authority | `prepare-821-flow-active-convergence.mjs` now emits the existing Flow lifecycle listeners directly inside their private app-owner lexical scope on the first pass. The historical boot-scope prepare is provenance-only and must not return to canonical build reachability. |
| `prepare-810-learning-budget-fix.mjs` as a corrective operator-precedence rewrite | retired from build authority | `prepare-810-learning-engine.mjs` now emits the already-proven parenthesized daily/session exposure-budget comparisons directly. The historical fix remains provenance-only; Deep Compatibility Chromium + iPhone-like WebKit keep the behavior release-blocking. |
| `prepare-810-learning-entry-fix.mjs` as a corrective Settings-row relocation | retired from build authority | `prepare-810-learning-settings.mjs` now emits the already-proven top-level primary Settings row, selector scope, global summary lookup and Settings-entry diagnostic directly. The historical fix remains provenance-only; later Settings convergence and dual-engine learning behavior stay release-blocking. |
