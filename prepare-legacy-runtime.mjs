...
function regexOnce(src,re,to,label){const flags=re.flags.includes('g')?re.flags:re.flags+'g',m=src.match(new RegExp(re.source,flags))||[];if(m.length!==1)throw new Error(`AXIS legacy sanitizer: ${label} expected once, found ${m.length}`);return src.replace(re,to)}
...
function patchActiveSetValue(kind,val){if(!draft.length)return;const s=draft[sel];if(kind==='weight')s.weight=val==null?null:Math.max(0,Math.min(1000,Math.round(Number(val||0)*100)/100));else s.reps=val==null?null:Math.max(1,Math.min(300,Math.round(Number(val)||1)));const host=$('#v8Sets'),row=host?.querySelectorAll?.('.v8SetRow')?.[sel];if(row){const bs=Array.from(row.querySelectorAll('span>b'));if(bs[0])bs[0].textContent=fmt(s.weight);if(bs[1])bs[1].textContent=fmt(s.reps);const eq=selected(),prevEvent=eq?last(eq.id,editingId):null,prev=prevEvent?setsOf(prevEvent):[],em=row.querySelector('em');if(em)em.textContent=deltaText(s,sel,prev)}syncHidden(draft);syncRecordingControls();emitRecording('axis:recording-change')}
...
function selectRecordingSet(i){if(!draft.length)return;sel=Math.max(0,Math.min(draft.length-1,Number(i)||0));const host=$('#v8Sets');Array.from(host?.querySelectorAll?.('.v8SetRow')||[]).forEach((row,n)=>row.classList.toggle('active',n===sel));syncRecordingControls();emitRecording('axis:recording-change')}
...
for(const file of stableFiles){
  ...
  if(file==='v61.js'){
    ...
    src=textOnce(src,"$('#strengthFields')?.classList.add('v8-hide-core');box.classList.remove('hidden');sel=","$('#strengthFields')?.classList.add('v8-hide-core');box.classList.remove('hidden');box.dataset.axisEquipment=String(e.id||e.name||'');sel=",'v61 recorder equipment identity');recordingOwnerFixes++;
    src=textOnce(src,"const nm=$('#equipmentName');if(nm)new MutationObserver(()=>{const e=selected();if(e?.type==='strength')prepare(e.id);else hideSets()}).observe(nm,{childList:true,subtree:true});","const nm=$('#equipmentName');if(nm)new MutationObserver(()=>{const e=selected(),box=$('#v8Sets'),key=String(e?.id||e?.name||'');if(e?.type==='strength'){if(box&&!box.classList.contains('hidden')&&box.dataset.axisEquipment===key)return;prepare(e.id)}else hideSets()}).observe(nm,{childList:true,subtree:true});",'v61 semantic equipment observer');recordingOwnerFixes++;
  }
...
for(const file of stableFiles){
  ...
  if(file==='v61.js'){
    ...
    if(!src.includes("box.dataset.axisEquipment=String(e.id||e.name||'')"))throw new Error('AXIS legacy sanitizer: recorder equipment identity missing');
    if(!src.includes("box.dataset.axisEquipment===key"))throw new Error('AXIS legacy sanitizer: semantic equipment observer guard missing');
...
if(recordingOwnerFixes!==7)throw new Error(`AXIS legacy sanitizer: expected 7 recording-owner fixes, found ${recordingOwnerFixes}`);
...
console.log(`[AXIS] recording converged · ${recordingOwnerFixes} core fixes · ${setBridgeOwnerFixes} set-bridge retirements · ${retiredOwnerFixes} v879 retirements · native event-free compatibility sync · semantic equipment dedupe`);
