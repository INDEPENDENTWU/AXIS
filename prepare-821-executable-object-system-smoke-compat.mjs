...
/* Keep the assertion unchanged, but expose the final execution/schema owner and
   set-plan visibility to distinguish resolver drift from a later v61 repaint. */
...
const familyOpenDiag="await page.evaluate(x=>window.__AXIS_QUICK_RECORD__.openFor(x),id);await page.waitForTimeout(320);const familyRoute=await page.evaluate(([targetName,targetKind,targetId])=>{const recorder=document.querySelector('#axis818MetricRecorder'),sets=document.querySelector('#v8Sets');return{targetName,targetKind,targetId,scanShown:document.querySelector('#scanSheet')?.classList.contains('show')||false,quickShown:document.querySelector('#quickRecordSheet')?.classList.contains('show')||false,pickerShown:document.querySelector('#eqSheet')?.classList.contains('show')||false,equipmentName:document.querySelector('#equipmentName')?.textContent?.trim()||'',objectMode:window.__AXIS_EXECUTABLE_OBJECTS__?.modeForEq?.(targetId)||null,flowMode:window.__AXIS_FLOW_RUNTIME__?.executionForRecording?.(targetId)||null,flowCurrent:window.__AXIS_FLOW_RUNTIME__?.current?.()||null,schema:(window.__AXIS_OBJECT_TRUTH__?.schemaForEq?.(targetId)||[]).map(x=>x?.key||x?.id),recorderShown:recorder?.classList.contains('show')||false,recorderSetPlanOwned:recorder?.dataset.axis821SetPlanOwned||'',setPlanShown:!!sets&&!sets.classList.contains('hidden'),kinds:[...document.querySelectorAll('#axis818MetricRecorder [data-axis821-kind]')].map(x=>x.dataset.axis821Kind),metrics:[...document.querySelectorAll('#axis818MetricRecorder [data-axis818-metric]')].map(x=>x.dataset.axis818Metric)}} ,[name,kind,id]);console.log(`[AXIS 8.21 Object system ${ENGINE}] family route ${name} ${JSON.stringify(familyRoute)}`);await page.waitForFunction(([n,k])=>document.querySelector('#scanSheet')?.classList.contains('show')&&document.querySelector('#equipmentName')?.textContent?.trim()===n&&document.querySelector(`#axis818MetricRecorder [data-axis821-kind=\"${k}\"]`),[name,kind],{timeout:4000});";
...
console.log('[AXIS 8.21 Executable Object smoke compat] PASS · inherited physical editor uses compact property sheet · continuous pace resolves to existing timed Active owner · generic count family avoids set-plan-owned sets · final execution-owner diagnostics enabled');
