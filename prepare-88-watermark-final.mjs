...
const fail=m=>{throw new Error(`AXIS 8.8 watermark final: ${m}`)};
...
// v85: visible switch owner. Add Time and keep its legacy preview/media path consistent.
{
  ...
  src=once(src,
    ...
    'v85 time writer');
  const render=[
    "function renderWm(){const p=wmPrefs(),g=latestGeo||prefs().v85LastGeo;",
    "for(const [id,on] of [['v85WmName',p.name],['v85WmData',p.data],['v85WmLocation',p.location],['v85WmTime',p.time]])$('#'+id)?.setAttribute('aria-checked',String(on));",
    "const b=$('#v85WmPreview');if(!b)return;b.style.left=p.pos.endsWith('r')?'auto':'18px';b.style.right=p.pos.endsWith('r')?'18px':'auto';b.style.top=p.pos.startsWith('t')?'18px':'auto';b.style.bottom=p.pos.startsWith('t')?'auto':'18px';",
    "$('#v85WmNamePreview').style.display=p.name?'block':'none';$('#v85WmDataPreview').style.display=p.data?'block':'none';",
    "const d=new Date(),time=`${d.getFullYear()}.${pad(d.getMonth()+1)}.${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}`,meta=p.time?time:'';",
    "$('#v85WmMetaPreview').textContent=meta;$('#v85WmMetaPreview').style.display=meta?'block':'none'}"
  ].join('');
  src=regexOnce(src,/function renderWm\(\)\{[\s\S]*?\}\nfunction wmData/,render+'\nfunction wmData','v85 preview contract');
  src=regexOnce(src,/const d=new Date\(ts\|\|now\(\)\),meta=[\s\S]*?;c\.save\(\);/,
    "const d=new Date(ts||now()),time=`${d.getFullYear()}.${pad(d.getMonth()+1)}.${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}`,meta=p.time?time:'';c.save();",
    'v85 legacy time contract');
  syntax(src,FILE);write(FILE,src);
}

// v876: compatibility watermark writer has already had coordinates retired by prepare-88-convergence.
// It only needs to honor the new Time preference too.
{
  ...
  src=once(src,
    ...
    'v876 preference shape');
  src=once(src,'c.fillText(time,x,yy);','if(p.time)c.fillText(time,x,yy);','v876 time media writer');
  syntax(src,FILE);write(FILE,src);
}

// v8710: one final visible preview + final-photo watermark owner.
{
  ...
  src=once(src,
    ...
    'v8710 preference shape');
  src=once(src,'.v8710LocationMeta{margin-top:12px;color:var(--dim);font-size:var(--axis-ui,15px);line-height:1.45}',
  ...
  src=regexOnce(src,/function rankInfo\(x\)\{[\s\S]*?\}\nasync function reverse\(g,L=selectedLang\(\)\)\{[\s\S]*?\}/,resolver,'precise place resolver');
  src=once(src,
    "function freshGeo(){const g=pref().geo;if(!pref().location||!navigator.geolocation)return Promise.resolve(g);",
    "function freshGeo(force=false){const g=pref().geo;if((!force&&!pref().location)||!navigator.geolocation)return Promise.resolve(g);",
    'forced locate geolocation');
  src=once(src,"async function refreshPlace(){const g=await freshGeo();await reverse(g,selectedLang());render();return g}","async function refreshPlace(force=false){const g=await freshGeo(force);await reverse(g,selectedLang());render();return g}",'forced precise refresh');
  src=once(src,"if(e.target.closest('#v876Locate'))setTimeout(()=>refreshPlace(),500)","if(e.target.closest('#v876Locate'))setTimeout(()=>refreshPlace(true),0)",'remove fixed locate delay');
  const helpers=[
  ...
  src=regexOnce(src,/function currentName\(\)\{[^\n]*\}\nfunction coord\(g\)\{[^\n]*\}/,helpers,'final preview helpers');
  src=once(src,
    ...
    'final preview rows');
  const render=[
    "function render(){ensure();sync();const p=pref(),name=currentName(),data=previewData(),time=timeText();",
    "if($('#v8710LocationMeta'))$('#v8710LocationMeta').textContent=p.place||'尚未获取位置';if($('#v876LocationName'))$('#v876LocationName').textContent=p.place||'未获取';",
    "const set=(id,on,text)=>{const el=$(id);if(!el)return;el.textContent=text||'';el.style.display=on&&text?'block':'none'};",
    "set('#v8710WmName',p.name,name);set('#v8710WmData',p.data,data);set('#v8710WmLoc',p.location,p.place);set('#v8710WmTime',p.time,time)}"
  ].join('');
  src=regexOnce(src,/function render\(\)\{[\s\S]*?\}\nfunction dbGet/,render+'\nfunction dbGet','final preview renderer');
  const stamp=[
    "async function stamp(blob,e){const url=URL.createObjectURL(blob),img=new Image();await new Promise((ok,no)=>{img.onload=ok;img.onerror=no;img.src=url});const cv=D.createElement('canvas');cv.width=img.naturalWidth;cv.height=img.naturalHeight;const c=cv.getContext('2d',{alpha:false});c.imageSmoothingEnabled=true;c.imageSmoothingQuality='high';c.drawImage(img,0,0,cv.width,cv.height);URL.revokeObjectURL(url);",
    "const p=pref(),L=p.resolvedLang,W=cv.width,H=cv.height,base=Math.max(20,Math.round(W*.0215)),big=Math.max(25,Math.round(W*.027)),pd=Math.max(22,Math.round(W*.03));c.save();c.globalAlpha=Math.max(.10,p.opacity/100);c.fillStyle='#737cff';c.textAlign='center';c.textBaseline='middle';c.font=`800 ${Math.max(74,Math.round(W*.12))}px -apple-system,BlinkMacSystemFont,Arial`;c.fillText('A X I S',W/2,H*.48);c.globalAlpha=1;c.fillStyle='rgba(115,124,255,.72)';c.fillRect(W*.12,H*.555,W*.76,Math.max(2,Math.round(W*.0025)));",
    "const name=L==='en'?englishName(e.name||'TRAINING').toUpperCase():(e.name||'训练'),time=timeText(e.time||Date.now()),data=eventData(e,L),rows=[];if(p.name)rows.push({text:name,size:big,weight:680,color:'#fff'});if(p.data&&data)rows.push({text:data,size:base,weight:580,color:'rgba(255,255,255,.96)'});if(p.location&&p.place)rows.push({text:p.place,size:base,weight:540,color:'rgba(255,255,255,.91)'});if(p.time)rows.push({text:time,size:base,weight:540,color:'rgba(255,255,255,.86)'});",
  ...
  src=regexOnce(src,/async function stamp\(blob,e\)\{[\s\S]*?\}\nasync function newest/,stamp+'\nasync function newest','final photo watermark');
  src=once(src,"old:{opacity:Number(p.v876WmOpacity)||18,pos:p.v85WmPos||'br',location:p.v85WmLocation!==false}","old:{opacity:Number(p.v876WmOpacity)||18,pos:p.v85WmPos||'br',name:p.v85WmName!==false,data:p.v85WmData!==false,location:p.v85WmLocation!==false,time:p.v85WmTime!==false}",'pre-save preference snapshot');
  src=once(src,"m.prefs.v876WmOpacity=4;m.prefs.v85WmPos='bl';m.prefs.v85WmLocation=false;write(META,m);setTimeout(()=>finishStamp(a),0)","m.prefs.v876WmOpacity=4;m.prefs.v85WmPos='bl';m.prefs.v85WmName=false;m.prefs.v85WmData=false;m.prefs.v85WmLocation=false;m.prefs.v85WmTime=false;write(META,m);setTimeout(()=>finishStamp(a),0)",'legacy optional metadata suppression');
  src=once(src,"m.prefs.v876WmOpacity=a.old.opacity;m.prefs.v85WmPos=a.old.pos;m.prefs.v85WmLocation=a.old.location;write(META,m);render()","m.prefs.v876WmOpacity=a.old.opacity;m.prefs.v85WmPos=a.old.pos;m.prefs.v85WmName=a.old.name;m.prefs.v85WmData=a.old.data;m.prefs.v85WmLocation=a.old.location;m.prefs.v85WmTime=a.old.time;write(META,m);render()",'post-save preference restore');
  src=once(src,"D.addEventListener('input',e=>{if(e.target.id==='v877OpacityRange'||e.target.id==='v876OpacityRange')setTimeout(render,0)},true);","D.addEventListener('input',e=>{if(e.target.id==='v877OpacityRange'||e.target.id==='v876OpacityRange')setTimeout(render,0)},true);D.addEventListener('click',e=>{if(e.target.closest('#v85WmName,#v85WmData,#v85WmLocation,#v85WmTime'))setTimeout(render,0)},false);",'preview switch repaint');
  if(/toFixed\(6\).*±|LAT |LON |纬度 .*经度/.test(src))fail('raw coordinate presentation survived final watermark owner');
  syntax(src,FILE);write(FILE,src);
}

console.log('[AXIS 8.8 watermark] final convergence passed · four switches · immediate precise OSM place · no raw coordinates');