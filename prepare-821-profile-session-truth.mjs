...
`;
 /* Reuse the already-proven canonical app lexical boundary used by the 8.21
    Object metric override layer. app.js starts with the canonical app IIFE;
    later appended IIFEs are projection modules and must never receive state/uid
    helpers that belong to the app owner. */
 const stateAt=s.indexOf('let state={'),close=s.indexOf('})();');
 if(stateAt<0||close<0||close<=stateAt)fail('canonical app lexical owner missing');
 s=s.slice(0,close)+runtime+s.slice(close);
...
console.log('[AXIS 8.21 Profile / Goal Session truth] PASS · canonical app lexical scope · existing Profile owners preserved · optional waist/targets additive · normal + Flow Session creation snapshots current context once · legacy absence preserved · no new store/Encounter/Active/Flow owner');
