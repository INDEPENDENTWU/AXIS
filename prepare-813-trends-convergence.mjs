...
const fail=m=>{throw new Error(`[AXIS 8.13.1 trends convergence] ${m}`)};
...
if(!html.includes('id="v811StateField"'))fail('8.11 trends surface must exist before 8.13.1 convergence');
...
const view=`<section class="view" id="insightsView" data-axis-trends-owner="v813-trends-field">
      ...
      <div class="v813Empty" id="v813Empty" hidden>暂无训练记录</div>
      <div class="v813Field" id="v813Field">
        ...
        <div class="v813Readout" aria-live="polite">
          ...
          <p class="v813Insight" id="v813Insight" hidden></p>
...
if((html.match(/data-axis-trends-owner="v813-trends-field"/g)||[]).length!==1)fail('8.13.1 visible owner missing or duplicated');
...
for(const label of ['左右滑动查看','点一下展开这次训练','留下几次训练后','留下训练记录后','当前状态','这次让什么发生了','下一针','状态场'])if(view.includes(label))fail(`user-facing helper copy survived: ${label}`);
fs.writeFileSync(INDEX,html);

// 8.13.1 keeps one visible Trends owner. Historical v84 cannot wrap the new surface.
...
// Source must already contain the 8.13.1 tap/scrub handoff and live projection lifecycle.
let trends=fs.readFileSync(TRENDS,'utf8');
for(const needle of ["if(!s.drag)return;","version:'8.13.1'","__AXIS_8131_EVOLUTION_FOUNDATION__","navigation-capture + pageshow + explicit-refresh","const CORE='axis_v60_state',META='axis_v8_meta'","暂无训练记录"]){if(!trends.includes(needle))fail(`8.13.1 Trends source missing ${needle}`)}
for(const forbidden of ['左右滑动查看','点一下展开这次训练','留下几次训练后','继续留下相同动作的记录'])if(trends.includes(forbidden))fail(`retired helper copy returned: ${forbidden}`);
try{new Function(trends)}catch(e){fail(`Trends syntax ${e.message}`)}
...
console.log('[AXIS 8.13.1 trends convergence] PASS · live sealed-session projection · direct copy · meta-backed fingerprints · same-day bearings · read-only Evolution foundation · one visible owner');
