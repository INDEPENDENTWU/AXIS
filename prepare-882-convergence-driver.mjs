...
const homeStart="  const homeModel=`";
const homeTail="`;\n  src=once(src,'function render(){',homeModel+'function render(){','home state model insertion');";
const hs=src.indexOf(homeStart),he=src.indexOf(homeTail,hs);
if(hs<0||he<0)throw new Error(`AXIS 8.8.2 convergence driver: home compiler block missing · ${hs}/${he}`);
const homeReplacement="  const homeModel=read('compiler/882-home-model.fragment.js');\n  src=once(src,'function render(){',homeModel+'function render(){','home state model insertion');";
src=src.slice(0,hs)+homeReplacement+src.slice(he+homeTail.length);
