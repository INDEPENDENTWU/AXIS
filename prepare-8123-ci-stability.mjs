...
}
{
 ...
 const settingsStart="assert.equal((await page.locator('#v810ConfigSummary').innerText()).trim(),'自定'";
 const settingsEnd="await page.locator('#settingsSheet [data-close=\"settingsSheet\"]').click();";
 const settingsCurrent=`assert.ok((await page.locator('#v810ConfigSummary').innerText()).trim().length>0,'top-level Learning Schedule summary is empty');for(const [k,v] of [['track','daily'],['cadence','long'],['level','progress'],['dailyTarget','12']])assert.equal(await page.locator(\`#v810SpeakControls [data-v810-key="\${k}"][data-v810-value="\${v}"]\`).evaluate(el=>el.classList.contains('active')),true,\`\${k} detail value is not reflected inside Learning Schedule\`);await page.locator('#v810ConfigEntry').click();await page.waitForFunction(()=>!document.querySelector('#v813LearningGate')?.classList.contains('open'));await page.locator('#settingsSheet [data-close="settingsSheet"]').click();`;
 s=convergeRange(s,settingsStart,settingsEnd,settingsCurrent,'top-level Learning Schedule summary is empty','8.10 current inline Learning Settings close + summary');
 write(f,s);
}
{
