...
}
{
 ...
 s=converge(s,"assert.equal((await page.locator('#v810ConfigSummary').innerText()).trim(),'自定','top-level Learning Schedule summary must stay native and compact');for(const [k,v] of [['track','daily'],['cadence','long'],['level','progress'],['dailyTarget','12']])assert.equal(await page.locator(`#v810SpeakControls [data-v810-key=\"${k}\"][data-v810-value=\"${v}\"]`).evaluate(el=>el.classList.contains('active')),true,`${k} detail value is not reflected inside Learning Schedule`);await page.locator('#v810ConfigPanel [data-v810-config-close]').click();await page.locator('#settingsSheet [data-close=\"settingsSheet\"]').click();","assert.ok((await page.locator('#v810ConfigSummary').innerText()).trim().length>0,'top-level Learning Schedule summary is empty');for(const [k,v] of [['track','daily'],['cadence','long'],['level','progress'],['dailyTarget','12']])assert.equal(await page.locator(`#v810SpeakControls [data-v810-key=\"${k}\"][data-v810-value=\"${v}\"]`).evaluate(el=>el.classList.contains('active')),true,`${k} detail value is not reflected inside Learning Schedule`);await page.locator('#v810ConfigEntry').click();await page.waitForFunction(()=>!document.querySelector('#v813LearningGate')?.classList.contains('open'));await page.locator('#settingsSheet [data-close=\"settingsSheet\"]').click();",'8.10 current inline Learning Settings close + summary');
 write(f,s);
}
{
