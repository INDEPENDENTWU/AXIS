...
}
{
 ...
 s=converge(s,"for(const k of ['mode','track','cadence','level','dailyTarget'])assert.ok(await page.locator(`#v810SpeakControls [data-v810-key=\"${k}\"]`).count()>=4,`missing ${k} choices`);","for(const k of ['track','cadence','level','dailyTarget'])assert.ok(await page.locator(`#v810SpeakControls [data-v810-key=\"${k}\"]`).count()>=4,`missing ${k} choices`);assert.equal(await page.locator('#v810SpeakControls [data-v810-key=\"mode\"]').count(),0,'retired learning mode controls returned');",'8.10 retired mode group');
 const summaryOld="assert.equal((await page.locator('#v810ConfigSummary').innerText()).trim(),'自定','top-level Learning Schedule summary must stay native and compact');";
 const summaryAlt="assert.equal((await page.locator('#v810ConfigSummary').innerText()).trim(),'智能 · 混合','8.12 top-level Learning Schedule should summarize purpose + practice method while fine-tune remains visible inside the panel');";
 const summaryCurrent="assert.ok((await page.locator('#v810ConfigSummary').innerText()).trim().length>0,'top-level Learning Schedule summary is empty');";
 if(s.includes(summaryOld))s=s.replace(summaryOld,summaryCurrent);else if(s.includes(summaryAlt))s=s.replace(summaryAlt,summaryCurrent);else if(!s.includes(summaryCurrent))fail('8.10 current Learning summary assertion shape missing');
 s=converge(s,"await page.locator('#v810ConfigPanel [data-v810-config-close]').click();","await page.locator('#v810ConfigEntry').click();await page.waitForFunction(()=>!document.querySelector('#v813LearningGate')?.classList.contains('open'));",'8.10 inline Learning Settings close');
 write(f,s);
}
{
