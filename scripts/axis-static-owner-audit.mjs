...
/* app.js retains the baseline DOM0 handoff, then the compiled 8.16 presentation
   entry deterministically supersedes that one property. The default-entry sentinel
   is intentionally `photo`: since 8.18 the canonical app owner interprets that
   sentinel through captureDefaultMode / captureLastMode. Scan sampling (3/5 sec)
   is a separate preference and must never be reused as the camera mode resolver.
   Both entry layers delegate to the same lifecycle owner; neither owns camera,
...
const finalEntry=unique.filter(x=>x.text.includes("dataset.axisCaptureEntryOwner='v816-capture-entry-seal'")&&x.text.includes("openCanonicalCamera('photo',null,false)"));
assert.equal(finalEntry.length,1,`expected one final v816 canonical default-entry delegation, got ${finalEntry.length}`);
...
const finalWrapper=src.match(/function open\(\)\{const c=window\.__AXIS_CAPTURE__;[^}]+\}/)?.[0]||'';
assert.equal(finalWrapper.includes('__AXIS_CAPTURE_PREF__'),false,'final Capture entry regressed to the legacy scan-sampling preference bridge');
assert.equal(/openCanonicalCamera\((?:preferred\(\)|['"]?[35]['"]?)/.test(finalWrapper),false,'final Capture entry passes scan sampling as camera mode');
...
console.log('[AXIS static owner audit] PASS · one camera lifecycle owner · baseline + final canonical default entry delegate deterministically · scan 3/5 isolated · Quick Record delegates · no entry owns camera lifecycle');
