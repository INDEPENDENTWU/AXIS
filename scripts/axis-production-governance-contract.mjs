...
const handoff=(owners.owners||[]).find(x=>x.capability==='evolution-replay-evidence-continuity-823');if(!['presentation-handoff-production-sealed','presentation-handoff-release-candidate'].includes(handoff?.status)||handoff?.storage!=='none')fail('8.23 continuity handoff owner drift');if(!String(handoff?.notes||'').includes('may not persist selection'))fail('8.23 continuity notes lost transient boundary');
...
if(CURRENT==='8.24'){
  if(handoff?.status!=='presentation-handoff-production-sealed'||e.status!=='production-sealed-8.23-inherited')fail('8.24 must inherit Production-sealed 8.23 Replay Evidence Continuity');
