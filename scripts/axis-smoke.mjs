...
async function verifyIdleEntry(page){if(await page.locator('#activeHome').isVisible())return;assert.ok(await page.locator('#idleHome').isVisible());assert.ok(await page.locator('#dock').isVisible());assert.ok(await page.locator('#scanBtn').isVisible());assert.ok(await page.locator('#quickRecordBtn').isVisible());await measuredClick(page,'#quickRecordBtn');try{await wait(page,()=>document.querySelector('#quickRecordSheet')?.classList.contains('show'),800)}catch(e){const diag=await page.evaluate(()=>{const q=document.querySelector('#quickRecordBtn'),s=document.querySelector('#quickRecordSheet'),d=document.querySelector('#dock');return{quickExists:!!q,quickOnclick:typeof q?.onclick,quickOnclickSource:String(q?.onclick||'').slice(0,500),sheetClass:s?.className||null,dockClass:d?.className||null,open:[...document.querySelectorAll('.sheetWrap.show')].map(x=>x.id),customSection:!!document.querySelector('#v882QuickMine'),recentHtml:(document.querySelector('#v8Recent')?.innerHTML||'').slice(0,500)}});console.error('[AXIS Quick Record cold-boot diagnostic]',JSON.stringify(diag,null,2));throw e}await measuredClick(page,'#quickClose',180)}
