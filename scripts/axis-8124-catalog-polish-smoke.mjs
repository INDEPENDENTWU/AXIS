...
try{
 ...
 console.log(`[AXIS 8.12.4 polish ${ENGINE}] expanded Learning + Cloud/AI share native Settings edges and Sound control rhythm`);
 ...
 await tap(page.locator('#settingsBtn'));await page.waitForFunction(()=>document.querySelector('#settingsSheet')?.classList.contains('show'));await page.waitForFunction(()=>document.querySelector('#v8710Audio'),undefined,{timeout:3000});
 const sound=await page.evaluate(()=>{const source=document.querySelector('#v8710Audio');const clone=source.cloneNode(true);clone.id='v8124SoundReferenceProbe';Object.assign(clone.style,{display:'block',position:'fixed',left:'-10000px',top:'0',width:'320px',visibility:'hidden',pointerEvents:'none'});document.body.appendChild(clone);const button=clone.querySelector('#v8710On button'),label=clone.querySelector('.v8710Top span'),br=button.getBoundingClientRect();const out={buttonHeight:br.height,labelFont:parseFloat(getComputedStyle(label).fontSize||0)};clone.remove();return out});
 assert.ok(sound.buttonHeight>0,`Sound geometry probe unavailable: ${JSON.stringify(sound)}`);assert.equal(sound.labelFont,15,`Sound label reference drifted: ${sound.labelFont}`);
 ...
 const learning=await page.evaluate(()=>{const gate=document.querySelector('#v813LearningGate'),fold=gate?.querySelector(':scope>.v8711Fold'),group=gate?.querySelector('.v811CoreGroup'),button=gate?.querySelector('.v811CoreOptions button'),label=gate?.querySelector('.v811CoreHead span');const b=x=>{const c=getComputedStyle(x),r=x.getBoundingClientRect();return{left:r.left,right:r.right,height:r.height,font:parseFloat(c.fontSize||0)}};return{gate:b(gate),foldPadding:[parseFloat(getComputedStyle(fold).paddingLeft||0),parseFloat(getComputedStyle(fold).paddingRight||0)],group:b(group),button:b(button),label:b(label)}});
 ...
 assert.ok(near(learning.button.height,sound.buttonHeight,.5),`Learning option height ${learning.button.height} vs Sound ${sound.buttonHeight}`);assert.ok(near(learning.label.font,sound.labelFont,.1),`Learning label font ${learning.label.font} vs Sound ${sound.labelFont}`);
 await tap(page.locator('#v810ConfigEntry'));await page.waitForFunction(()=>!document.querySelector('#v813LearningGate')?.classList.contains('open'));
 ...
 const service=await page.evaluate(()=>{const gate=document.querySelector('#v813ServiceGate'),fold=gate?.querySelector(':scope>.v8711Fold'),group=gate?.querySelector('.axis8122Group'),button=gate?.querySelector('.axis8122Grid button'),label=gate?.querySelector('.axis8122Head span');const b=x=>{const c=getComputedStyle(x),r=x.getBoundingClientRect();return{left:r.left,right:r.right,height:r.height,font:parseFloat(c.fontSize||0)}};return{gate:b(gate),foldPadding:[parseFloat(getComputedStyle(fold).paddingLeft||0),parseFloat(getComputedStyle(fold).paddingRight||0)],group:b(group),button:b(button),label:b(label)}});
 ...
 assert.ok(near(service.button.height,sound.buttonHeight,.5),`Cloud/AI option height ${service.button.height} vs Sound ${sound.buttonHeight}`);assert.ok(near(service.button.height,learning.button.height,.5),`Learning / Cloud touch geometry diverged: ${learning.button.height} vs ${service.button.height}`);assert.ok(near(service.label.font,sound.labelFont,.1),`Cloud/AI label font ${service.label.font} vs Sound ${sound.labelFont}`);
