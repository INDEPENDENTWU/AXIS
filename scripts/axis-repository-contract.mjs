...
const allowedRetirementBaselines=new Set([CURRENT,SEALED,'8.21']);
...
if(['8.22','8.23'].includes(CURRENT)){
  const replay=(owners.owners||[]).find(x=>x.capability==='evolution-replay-822');
  if(!String(replay?.status||'').includes('derived-read-only')||replay?.storage!=='none')fail('8.22 Evolution Replay owner registry drift');
}
if(CURRENT==='8.23'){
  const handoff=(owners.owners||[]).find(x=>x.capability==='evolution-replay-evidence-continuity-823');
  if(handoff?.status!=='presentation-handoff-release-candidate'||handoff?.storage!=='none')fail('8.23 Replay Evidence handoff owner registry drift');
}
...
if(['8.22','8.23'].includes(CURRENT)&&!lifecycle.includes("await import('./prepare-822-evolution-replay.mjs')"))fail('8.22 Replay is not reachable');
if(CURRENT==='8.23'&&!lifecycle.includes("await import('./prepare-823-replay-evidence-continuity.mjs')"))fail('8.23 Replay Evidence Continuity is not reachable after 8.22');
if(['8.22','8.23'].includes(CURRENT)){
  const release822=read('prepare-822-evolution-replay.mjs');if(!release822.includes("const FROM='8.21',VERSION='8.22'"))fail('8.22 release transition drift');
  if(!build.includes("'postbuild-822-evolution-replay-contract.mjs'"))fail('8.22 postbuild contract is not deterministic build authority');
}
if(CURRENT==='8.23'){
  const release823=read('prepare-823-replay-evidence-continuity.mjs');
  if(!release823.includes("const FROM='8.22',VERSION='8.23'"))fail('8.23 release transition contract drift');
  ...
  if((currentWorkflow.match(/node scripts\/axis-823-replay-evidence-continuity-smoke\.mjs/g)||[]).length!==2)fail('8.23 smoke must remain in the converged Current Release family in both engines');
  if(fs.existsSync('.github/workflows/axis-823-replay-evidence-continuity.yml'))fail('version-specific 8.23 automatic workflow fanout must not return');
}
...
const built819Identity=post818.includes("contract.publicVersion!=='8.19'")&&post818.includes("info.version!=='8.19'");
const built820Identity=post818.includes("contract.publicVersion!=='8.20'")&&post818.includes("info.version!=='8.20'");
const built821Identity=post818.includes("contract.publicVersion!=='8.21'")&&post818.includes("info.version!=='8.21'");
const built822Identity=post818.includes("contract.publicVersion!=='8.22'")&&post818.includes("info.version!=='8.22'");
const built823Identity=post818.includes("contract.publicVersion!=='8.23'")&&post818.includes("info.version!=='8.23'");
if(!source818Identity&&!built819Identity&&!built820Identity&&!built821Identity&&!built822Identity&&!built823Identity)fail('8.18 semantic contract has neither sealed source identity nor supported built identity');
...
const steps=[...stepBlock[1].matchAll(/'([^']+\.mjs)'/g)].map(match=>match[1]);if(!steps.length)fail('no deterministic build steps found');
const duplicateSteps=steps.filter((step,index)=>steps.indexOf(step)!==index);if(duplicateSteps.length)fail(`duplicate build steps: ${[...new Set(duplicateSteps)].join(', ')}`);
for(const step of steps)if(!fs.existsSync(step))fail(`build step does not exist: ${step}`);
const expectedSteps=CURRENT==='8.23'?86:85;if(steps.length!==expectedSteps)fail(`${CURRENT} must have exactly ${expectedSteps} deterministic top-level steps, found ${steps.length}`);
...
const history=['docs/history/8.3.3-stable-marker.txt','docs/history/production-restore-2026-08-12.txt','docs/history/deploy-trigger-8.10.3-2026-08-16.txt','docs/history/deploy-trigger-8.8.4-2026-08-16.md'];for(const path of history)if(!fs.existsSync(path))fail(`historical record missing: ${path}`);
for(const path of ['LATEST_RELEASE_833.txt','PRODUCTION_RESTORE_20260812.txt','deploy-trigger.txt','docs/deploy-trigger.md'])if(fs.existsSync(path))fail(`one-off historical marker remains: ${path}`);
const ignore=read('.gitignore');for(const artifact of ['axis-core.js','axis-style.css','axis-build.json','node_modules/','.env'])if(!ignore.includes(artifact))fail(`.gitignore missing ${artifact}`);
try{const tracked=new Set(execFileSync('git',['ls-files'],{encoding:'utf8'}).split(/\r?\n/).filter(Boolean));for(const artifact of ['axis-core.js','axis-style.css','axis-build.json'])if(tracked.has(artifact))fail(`generated artifact is tracked: ${artifact}`)}catch(error){if(String(error?.message||'').includes('[AXIS repository contract]'))throw error;console.warn('[AXIS repository contract] git tracked-file audit unavailable; filesystem contract still verified')}

const prepareCount=steps.filter(step=>step.startsWith('prepare-')).length,postbuildCount=steps.filter(step=>step.startsWith('postbuild-')).length;
