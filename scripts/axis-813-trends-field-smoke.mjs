...
try{
 ...
 const visibleState=await page.evaluate(()=>{const view=document.querySelector('#insightsView'),field=document.querySelector('#v813Field'),vp=document.querySelector('#v813Viewport');const snap=el=>{if(!el)return null;const r=el.getBoundingClientRect(),s=getComputedStyle(el);return{tag:el.tagName,id:el.id,cls:el.className,hidden:el.hidden,display:s.display,visibility:s.visibility,opacity:s.opacity,position:s.position,width:s.width,height:s.height,w:r.width,h:r.height,top:r.top,left:r.left}};const chain=[];let el=field;for(let i=0;el&&i<8;i++,el=el.parentElement)chain.push(snap(el));return{active:view?.classList.contains('active')||false,fieldCount:document.querySelectorAll('#v813Field').length,nodeCount:document.querySelectorAll('.v813Node').length,view:snap(view),field:snap(field),viewport:snap(vp),chain}});
