...
await page.locator('#v87Toggle').click();await page.waitForTimeout(620);const paused=(await page.locator('#v87Meta').innerText()).trim();await page.waitForTimeout(1050);assert.equal((await page.locator('#v87Meta').innerText()).trim(),paused);assert.equal((await page.locator('#v87Toggle').innerText()).trim(),'▶','single paused item did not expose canonical resume control');await page.locator('#v87Toggle').click();await page.waitForFunction(pausedText=>document.querySelector('#v87Meta')?.textContent?.trim()!==pausedText,paused,{timeout:1800});assert.notEqual((await page.locator('#v87Meta').innerText()).trim(),paused);
