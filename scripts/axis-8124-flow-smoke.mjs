...
try{
 ...
 const hold=page.locator('#finishHold');await hold.dispatchEvent('pointerdown',{pointerId:7,pointerType:'touch',isPrimary:true,buttons:1});await page.waitForTimeout(1180);await hold.dispatchEvent('pointerup',{pointerId:7,pointerType:'touch',isPrimary:true,buttons:0}).catch(()=>{});await page.waitForFunction(()=>{const c=JSON.parse(localStorage.getItem('axis_v60_state')||'{}');return !c.active&&c.sessions?.[0]?.end>c.sessions?.[0]?.start},undefined,{timeout:3000});
 await page.locator('#finishDone').click().catch(()=>{});await page.waitForTimeout(100);const completed=await page.evaluate(()=>({state:window.__AXIS_HOME_STATE__,session:JSON.parse(localStorage.getItem('axis_v60_state')||'{}').sessions?.[0]}));assert.ok(completed.session.end-completed.session.start>20*60000,'session end collapsed onto start');assert.equal(completed.state.scope,'complete');const facts=completed.state.meta;const match=facts.match(/开始\s+(\d\d:\d\d).*完成\s+(\d\d:\d\d)/);assert.ok(match,`start/end facts missing: ${facts}`);assert.notEqual(match[1],match[2],`displayed start/end collapsed: ${facts}`);
