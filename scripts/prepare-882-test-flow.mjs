...
const current=patchFile('scripts/axis-882-smoke.mjs',[
  [
    ...
    "const errors=[],consoleErrors=[];page.on('pageerror',e=>errors.push(String(e?.stack||e)));page.on('console',m=>{if(m.type()==='error')consoleErrors.push(m.text())});",
...
if(!current.includes('consoleErrors=[]'))throw new Error('[AXIS 8.8.2 test flow] browser console error capture missing');
