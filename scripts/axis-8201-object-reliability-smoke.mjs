...
const waitCore=async()=>{await page.waitForFunction(()=>window.__AXIS_CORE_INTERACTIVE__===true,undefined,{timeout:15000});await page.waitForFunction(()=>window.__AXIS_OBJECT_TRUTH__?.version==='8.18'&&window.__AXIS_820_EXECUTABLE_OBJECT__?.version==='8.20'&&window.__AXIS_8201_OBJECT_SYNC__?.liveSchema===true,undefined,{timeout:8000})};
...
const createFromSearch=async name=>{await openPicker();await search(name);const create=page.locator('#v873SmartResults [data-axis-create-custom]');assert.equal(await create.count(),1,`direct create missing for ${name}`);await tap(create);await page.waitForFunction(()=>document.querySelector('#customEqSheet')?.classList.contains('show'),undefined,{timeout:2500});assert.equal(await page.locator('#customName').inputValue(),name);await page.waitForFunction(()=>document.querySelectorAll('#axis818MetricEditor [data-axis818-metric-choice]').length>=9,undefined,{timeout:2500})};
...
try{
 ...
 await createFromSearch('靠墙站立');await keepOnly('duration');
