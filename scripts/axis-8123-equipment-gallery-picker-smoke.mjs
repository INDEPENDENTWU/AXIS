...
const chooseByText=async text=>{const canonical=page.locator('#v8710Cards button').filter({hasText:text}).first();if(await canonical.count()){await tap(canonical);return}const native=page.locator('#eqSheet [data-eq]').filter({hasText:text}).first();assert.equal(await native.count(),1,`picker item ${text} missing`);await tap(native)};
