...
const openPhotoReview=async()=>{await tap(page.locator('#scanBtn'));await page.waitForFunction(()=>document.querySelector('#scanSheet')?.classList.contains('show'));await page.locator('#photoInput').setInputFiles({name:'axis-photo.png',mimeType:'image/png',buffer:png});if(await page.locator('#v816CaptureDone').count()){await page.waitForFunction(()=>document.querySelectorAll('#v816DraftRail .v816DraftItem').length>0&&document.querySelector('#v816CaptureDone')?.disabled===false,undefined,{timeout:3500});assert.equal(await page.locator('#v816CaptureDone').isEnabled(),true,'gallery flow canonical Capture Field done action is not enabled after photo draft');await tap(page.locator('#v816CaptureDone'))}await page.waitForFunction(()=>!document.querySelector('#reviewStage')?.classList.contains('hidden'),undefined,{timeout:3500})};
...
try{
 ...
 const settings=await page.evaluate(()=>{const bw=el=>el?parseFloat(getComputedStyle(el).borderBottomWidth)||0:null,visible=el=>!!el&&el.offsetParent!==null;const learning=document.querySelector('#v813LearningGate>.settingLink'),service=document.querySelector('#v813ServiceGate>.settingLink'),report=document.querySelector('#reportBtn'),reminder=[...document.querySelectorAll('#settingsSheet button,#settingsSheet .settingLink')].find(el=>visible(el)&&((el.querySelector(':scope>span')?.textContent||el.textContent||'').replace(/\s+/g,'')).startsWith('提醒与声音'));return{learning:visible(learning)?bw(learning):null,service:visible(service)?bw(service):null,report:visible(report)?bw(report):null,reminder:bw(reminder),reminderText:reminder?.textContent||'',reportClass:report?.className||'',reportText:report?.textContent||''}});
 assert.notEqual(settings.learning,null,'visible 学习安排 row missing');assert.notEqual(settings.service,null,'visible 云端与AI row missing');assert.notEqual(settings.report,null,'visible 训练报告 row missing');assert.notEqual(settings.reminder,null,'visible 提醒与声音 row missing');assert.equal(settings.learning,0);assert.equal(settings.service,0);assert.equal(settings.report,0);assert.equal(settings.reminder,0);assert.match(settings.reportClass,/v8123ReportEntry/);assert.match(settings.reportText,/训练报告/);
 ...
 console.log(`[AXIS 8.12.3 gallery/picker ${ENGINE}] PASS · canonical draft→完成→review · stable repeated picker · Quick other catalog · multi-photo memory · divider-free Settings · report action preserved`);
