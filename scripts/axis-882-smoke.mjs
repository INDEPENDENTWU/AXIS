...
await page.waitForFunction(()=>window.__AXIS_882_CUES__?.length===1,undefined,{timeout:3200});assert.deepEqual(await page.evaluate(()=>window.__AXIS_882_CUES__),['item']);await page.waitForTimeout(1200);assert.deepEqual(await page.evaluate(()=>window.__AXIS_882_CUES__),['item'],'countdown zero repeated the cue');
if(VERSION==='8.24'){
  await page.waitForFunction(()=>{const t=document.querySelector('#v87Meta')?.textContent.trim()||'';return t.startsWith('预计 ')&&!t.includes('剩余')&&!t.includes('组')},undefined,{timeout:1800});
}else{
  await page.waitForFunction(()=>document.querySelector('#v87Meta')?.textContent.trim().startsWith('剩余 00:00'),undefined,{timeout:1000});
}
