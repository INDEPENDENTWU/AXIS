...
await page.reload({waitUntil:'domcontentloaded'});await ready();await page.waitForFunction(()=>document.querySelector('#v87Rest')?.classList.contains('v8101Opportunity'),undefined,{timeout:3500});
const metaBaseline=await page.evaluate(()=>localStorage.getItem('axis_v8_meta'));assert.equal(await page.evaluate(()=>window.__AXIS_8102_SPEAK_CALLS__),0,'paused opportunity autoplayed');
await page.locator('#v87Rest').click();await page.waitForFunction(()=>document.querySelector('#v891SpeakPanel')?.classList.contains('show')&&document.querySelector('#v891SpeakPanel')?.dataset.axis8102Source==='opportunity');
