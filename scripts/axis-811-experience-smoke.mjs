...
for(const gate of ['learningAtlas811','learningAtlasUnique811','learningDialogueFourTurn811','learningConnectedSpeech811','learningSpelling811','learningSettingsConverged811','learningLegacyDiagnosticsPreserved811','trendsStateField811','trendsGoalAware811','trendsEvidenceOnly811','trendsLocalFirst811'])if(info.gates?.[gate]!==true)fail(`manifest gate missing ${gate}`);
...
for(const needle of ["legacyPrefsPreserved:true","networkRequired:false","dialogue:'unit-specific-four-turn'",'legacy.richEnglish=456','legacy.totalUnits=492','legacy.phrases=()=>492','availableEnglish=5736','availableUnits=5772'])if(!runtime.includes(needle))fail(`runtime missing ${needle}`);
...
console.log(`[AXIS 8.11 experience smoke] PASS · ${audit.count} atlas units · ${audit.duplicateTargets} duplicate targets · legacy diagnostics/settings/trends contract green`);
