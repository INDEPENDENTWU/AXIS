...
const openAudio=async()=>{
  ...
  await page.waitForFunction(()=>document.querySelector('#v8710Session'),undefined,{timeout:1200});
};
...
console.log(`[AXIS ${ENGINE}] custom session reminder repaints without reload`);
await openAudio();
await page.locator('#v8710Session').click();
await page.waitForFunction(()=>document.querySelector('#v876TargetSheet')?.classList.contains('show'),undefined,{timeout:1200});
await page.locator('#v876TargetInput').fill('75');
await page.locator('#v876TargetApply').click();
assert.equal(await page.locator('#v876TargetSheet.show').count(),0,'duration sheet did not close after apply');
assert.equal((await page.locator('#v8710Session b').innerText()).trim(),'75分','current sound UI did not repaint custom duration synchronously');
const target=await page.evaluate(()=>{try{return Number(JSON.parse(localStorage.getItem('axis_v8_meta')||'{}').prefs?.v876SessionTarget)||0}catch{return 0}});
assert.equal(target,75,'custom session duration was not persisted');
...
console.log(`[AXIS ${ENGINE}] PASS · 75分 immediate repaint · 12px card/dock · 8px dock/nav · aligned content edges`);
