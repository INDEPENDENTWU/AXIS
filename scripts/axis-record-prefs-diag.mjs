...
const snapshot=label=>page.evaluate(l=>{
 ...
 const gate=q('#v8711RecordGate'),fold=gate?.querySelector('.v8711Fold'),second=q('#settingsSheet .settingsList.second'),scan=q('#scanSeconds'),btn=q('#scanSeconds [data-sec="5"]'),keep=q('#keepClipSwitch'),sheet=q('#settingsSheet .sheet');
 let center=null,hit=null;if(btn){const r=btn.getBoundingClientRect();center={x:r.left+r.width/2,y:r.top+r.height/2};const h=document.elementFromPoint(center.x,center.y);hit=h?{tag:h.tagName,id:h.id||'',class:String(h.className||''),text:String(h.textContent||'').trim().slice(0,60)}:null}
 ...
 const hiddenAncestors=[];if(btn){for(let p=btn.parentElement;p&&p!==document.documentElement;p=p.parentElement){const c=css(p);if(c.display==='none'||c.visibility==='hidden'||Number(c.opacity)===0||p.getBoundingClientRect().width===0||p.getBoundingClientRect().height===0)hiddenAncestors.push({id:p.id||'',class:String(p.className||''),display:c.display,visibility:c.visibility,opacity:c.opacity,box:box(p)})}}
 return{label:l,settingsClass:q('#settingsSheet')?.className||'',settingsScroll:sheet?.scrollTop||0,openGates:[...document.querySelectorAll('#settingsSheet .v8711SettingGate.open,#settingsSheet .axisConfigGate.open')].map(x=>x.id),gateClass:gate?.className||'',gate:info(gate),fold:info(fold),second:info(second),scan:info(scan),button:info(btn),keep:info(keep),center,hit,hiddenAncestors};
},label);
...
// Reproduce the same preceding Settings journey as the full matrix.
...
const target=page.locator('#scanSeconds [data-sec="5"]');
assert.ok(await target.isVisible(),`record preference control is not visible after gate opens: ${JSON.stringify(after)}`);
const r=await target.boundingBox();assert.ok(r&&r.width>0&&r.height>0,'record preference target has no geometry');
assert.ok(after.hit&&(['scanSeconds'].includes(after.hit.id)||/data-sec/.test(after.hit.text)||after.hit.tag==='BUTTON'),`record preference target center is covered: ${JSON.stringify(after.hit)}`);
...
console.log('[AXIS record-pref diagnostic] PASS · gate opens and control is actionable');
