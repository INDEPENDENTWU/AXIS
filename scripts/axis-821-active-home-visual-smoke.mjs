...
async function proveViewport(width,height,mobile){
 ...
 try{
  ...
  const g=await page.evaluate(()=>{
   ...
   const hr=R(host),pr=R(parent),cr=R(clock),tr=R(toggle),rr=R(primary),ar=R(adjust),fr=R(fact),xr=R(controls),cs=getComputedStyle(host),as=getComputedStyle(adjust),ps=getComputedStyle(primary),ts=getComputedStyle(toggle),style=document.querySelector('#axis821ActiveStageRefineStyle')?.textContent||'';
   return{host:hr,parent:pr,clock:cr,toggle:tr,primary:rr,adjust:ar,fact:fr,controls:xr,backgroundColor:cs.backgroundColor,backgroundImage:cs.backgroundImage,boxShadow:cs.boxShadow,radius:parseFloat(cs.borderTopLeftRadius)||0,position:cs.position,adjustPosition:as.position,primaryRadius:parseFloat(ps.borderTopLeftRadius)||0,toggleRadius:parseFloat(ts.borderTopLeftRadius)||0,overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,stylePresent:!!style,marker:window.__AXIS_821_ACTIVE_HOME_VISUAL__,adjustText:adjust.textContent.trim(),clockText:clock.textContent.trim()};
  });
  ...
  console.log(`[AXIS 8.21 Active Home visual ${ENGINE}] PASS ${width}x${height} · flat rail · centered clock · balanced controls · isolated Adjust · no overflow`);
