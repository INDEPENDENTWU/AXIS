...
const contentEdge=()=>page.locator('#settingsSheet .sheet').evaluate(el=>{const r=el.getBoundingClientRect(),c=getComputedStyle(el);return{left:r.left+parseFloat(c.paddingLeft||0),right:r.right-parseFloat(c.paddingRight||0)}});
const rect=sel=>page.locator(sel).first().evaluate(el=>{const r=el.getBoundingClientRect();return{left:r.left,right:r.right,width:r.width,height:r.height}});
...
try{
 ...
 await page.waitForFunction(()=>document.querySelector('#v8710On'),undefined,{timeout:3000});
 ...
 const before=await trainingStores(),edge=await contentEdge(),referenceSeg=await rect('#v8710On'),referenceButton=await rect('#v8710On button');

 console.log(`[AXIS settings ${ENGINE}] Learning Schedule expands in place with native Settings detail geometry`);
 ...
 const learningBlock=await rect('#v813LearningGate .v811CoreGroup'),coreButton=await rect('#v811CoreLearning button');
 assert.ok(near(learningBlock.left,edge.left)&&near(learningBlock.right,edge.right),`Learning detail inset drifted: ${JSON.stringify({learningBlock,edge})}`);
 assert.ok(near(coreButton.height,referenceSeg.height),`Learning control rhythm differs from Sound: ${coreButton.height} vs ${referenceSeg.height}`);
 ...
 console.log(`[AXIS settings ${ENGINE}] Cloud + AI expands in place with the same Settings geometry; network remains user-invoked`);
 ...
 const serviceBlock=await rect('#v813ServiceGate .v813ServiceBlock'),serviceSeg=await rect('#v811CloudMode'),serviceButton=await rect('[data-v811-cloud="off"]');
 assert.ok(near(serviceBlock.left,edge.left)&&near(serviceBlock.right,edge.right),`Cloud/AI detail inset drifted: ${JSON.stringify({serviceBlock,edge})}`);
 assert.ok(near(serviceSeg.height,referenceSeg.height),`Cloud/AI segment rhythm differs from Sound: ${serviceSeg.height} vs ${referenceSeg.height}`);
 assert.ok(near(serviceButton.height,referenceButton.height),`Cloud/AI option height differs from Sound: ${serviceButton.height} vs ${referenceButton.height}`);
 ...
 console.log(`[AXIS settings ${ENGINE}] PASS · one Settings sheet · native detail insets/control rhythm · delegated fine tune preserved · explicit service network`);
}finally{
