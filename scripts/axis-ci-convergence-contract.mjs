...
const required=[
  '.github/workflows/axis-current-release-gate.yml','.github/workflows/axis-runtime-foundation-gate.yml','.github/workflows/axis-deep-compatibility-gate.yml','.github/workflows/axis-pr-run-convergence.yml','.github/workflows/axis-812-browser-gate.yml',
  'scripts/axis-current-release-contract.mjs','scripts/axis-runtime-foundation-contract.mjs','scripts/axis-deep-compatibility-contract.mjs','scripts/axis-812-browser-smoke.mjs','governance/ci-inventory.json','docs/CI_CONVERGENCE.md'
];
...
const specialist=fs.readFileSync('.github/workflows/axis-812-browser-gate.yml','utf8'),specialistSmoke=fs.readFileSync('scripts/axis-812-browser-smoke.mjs','utf8');
for(const n of ["paths:","scripts/axis-812-*.mjs","AXIS_ENGINE: chromium","AXIS_ENGINE: webkit","axis-812-browser-smoke.mjs"])if(!specialist.includes(n))fail(`8.12 path-scoped specialist lost ${n}`);
for(const n of ["totalAvailable===25716","dialogueTurns.short===4","dialogueTurns.full===8","dialogueTurns.immersive===12","r8123?.settingsMethod===false","groups===4","[data-v812-core=\"method\"]","state.overflow<=1","errors.length===0"])if(!specialistSmoke.includes(n))fail(`8.12 browser semantic contract lost ${n}`);
...
if(state?.baselineAutomaticWorkflowFamilyCount!==9||state?.baselineAutomaticWorkflowFamilies?.length!==9)fail('baseline automatic workflow family inventory drifted');
if(state?.pathScopedSpecialistWorkflows?.length!==1||state.pathScopedSpecialistWorkflows[0]?.path!=='.github/workflows/axis-812-browser-gate.yml'||state.pathScopedSpecialistWorkflows[0]?.preserve!==true)fail('path-scoped specialist inventory drifted');
if(state?.branchProtectionAudit?.requiredStatusChecks!==0)fail('branch-protection audit changed; re-evaluate retirements');
console.log('[AXIS CI convergence contract] PASS · 20 retired workflow files absent · 9 baseline responsibility families + preserved 8.12 path-scoped specialist · replacement coverage sealed');
