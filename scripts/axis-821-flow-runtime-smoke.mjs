...
await context.addInitScript(({A,B,C})=>{if(sessionStorage.getItem('__axis821_seeded__')==='1')return;const now=Date.now();localStorage.clear();localStorage.setItem('axis_v60_state',JSON.stringify({version:60,sessions:[],active:{id:'S-821',start:now-60000,events:[]},profile:{customEq:[A,B,C],memories:[]},prefs:{scanSeconds:3,captureDefaultMode:'photo',captureDefaultFacing:'environment'}}));localStorage.setItem('axis_v8_meta',JSON.stringify({events:{},prefs:{}}));sessionStorage.setItem('__axis821_seeded__','1')},{A,B,C});
...
const saveCurrent=async()=>{await tap(page.locator('#saveScan'))};
...
const currentPickerItem=async(id,label)=>{
 ...
 const search=page.locator('#eqSearch');assert.equal(await search.count(),1,'current equipment search missing');await search.fill(label);
 ...
 item=page.locator(`#v873SmartResults [data-v8124-pick="${id}"]:visible`).first();if(!(await item.count()))item=page.locator(`#v8124PickerContext [data-v8124-pick="${id}"]:visible`).first();if(!(await item.count()))item=page.locator(`#eqSheet [data-eq="${id}"]:visible`).first();if(!(await item.count()))item=page.locator('#v873SmartResults [data-v8124-pick]:visible').filter({hasText:label}).first();if(!(await item.count()))item=page.locator('#v8710Cards button:visible').filter({hasText:label}).first();return item;
};
const openCurrentFromQuick=async(id,label)=>{
 await page.evaluate(()=>document.querySelector('#quickRecordBtn')?.click());
 await page.waitForFunction(()=>document.querySelector('#quickRecordSheet')?.classList.contains('show'),undefined,{timeout:2200});
...
try{
 ...
 console.log(`[AXIS 8.21 Flow ${ENGINE}] temporary metric override delegates through real Quick Record lifecycle`);
 const handoffA=await page.evaluate(()=>window.__AXIS_FLOW_RUNTIME__.selectCurrent());assert.equal(handoffA.objectRef,A.id);await openCurrentFromQuick(A.id,A.name);
 ...
 console.log(`[AXIS 8.21 Flow ${ENGINE}] classic step remains v61-owned through real Quick Record lifecycle`);
 const handoffB=await page.evaluate(()=>window.__AXIS_FLOW_RUNTIME__.selectCurrent());assert.equal(handoffB.objectRef,B.id);await openCurrentFromQuick(B.id,B.name);await page.waitForTimeout(120);await saveCurrent();await waitEvent(B.id);
 ...
 console.log(`[AXIS 8.21 Flow ${ENGINE}] complete one-shot uses real Quick Record and creates no false Active`);
 const handoffC=await page.evaluate(()=>window.__AXIS_FLOW_RUNTIME__.selectCurrent());assert.equal(handoffC.objectRef,C.id);await openCurrentFromQuick(C.id,C.name);await page.waitForFunction(()=>document.querySelector('[data-axis818-metric="rating"]'),undefined,{timeout:2500});await page.locator('[data-axis818-metric="rating"]').fill('8');await saveCurrent();await waitEvent(C.id);await page.waitForTimeout(180);
 ...
 console.log(`[AXIS 8.21 Flow ${ENGINE}] PASS · same-store definition/run · reload · one-current-step · real Quick lifecycle · temporary override nonmutation · app-owned provenance · v61 classic authority · complete one-shot · no new storage`);
