...
function metricFacts(event){
  ...
  return keys.map(key=>{
    ...
    return{key,recorded,declaredInSchema:!!schema?.includes(key),value:recorded?clone(metrics[key]):null,definitionStatus:standard?'stable-standard-key':'encounter-key-only',definitionMissing:!standard};
  });
}
...
export function axisReportRangeBuild(sessions=[],range={}){
  ...
  for(const [sessionIndex,session] of selected.entries())for(const encounter of session.encounters)for(const fact of encounter.metricFacts)if(fact.recorded)metricObservations.push({sessionIndex,sessionId:session.id,sessionStart:session.start,encounterIndex:encounter.index,encounterId:encounter.id,time:encounter.time,key:fact.key,value:clone(fact.value),definitionStatus:fact.definitionStatus,definitionMissing:fact.definitionMissing,declaredInSchema:fact.declaredInSchema});
