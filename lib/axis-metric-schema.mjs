...
const BUILTIN=Object.freeze({
  ...
  duration:{id:'duration',type:'duration',label:'时间',unit:'min',priority:'primary',repeatability:'single',aggregation:'sum',presentation:'timer',min:0,step:1},
  ...
  pace:{id:'pace',type:'pace',label:'配速',unit:'min/km',priority:'secondary',repeatability:'single',aggregation:'average',presentation:'pace',min:0},
  intensity:{id:'intensity',type:'rating',label:'强度',unit:'/10',priority:'secondary',repeatability:'single',aggregation:'average',presentation:'rating',min:1,max:10,step:1},
  resistance:{id:'resistance',type:'number',label:'阻力',unit:'',priority:'secondary',repeatability:'single',aggregation:'average',presentation:'stepper',min:0,step:1},
  level:{id:'level',type:'number',label:'等级',unit:'',priority:'secondary',repeatability:'single',aggregation:'average',presentation:'stepper',min:0,step:1},
...
export function normalizeMetricDefinition(input,index=0){
  ...
  if(merged.extensions&&typeof merged.extensions==='object'&&!Array.isArray(merged.extensions))out.extensions=clone(merged.extensions);
  return out;
}
...
export function normalizeMetricSchema(input,{objectId=null,source='explicit'}={}){
  ...
  if(!raw||typeof raw!=='object'||!Array.isArray(raw.metrics)||!raw.metrics.length)fail('metric schema requires at least one metric');
...
export function resolveRecordingSurface(schema){
  ...
  const continuous=entries.length===0&&normalized.metrics.some(metric=>['duration','distance','pace','speed'].includes(metric.type)||['duration','distance','pace','speed'].includes(metric.id));
  return {
...
function valueForMetric(metric,source){
  ...
  if(metric.type==='boolean')return Boolean(value);
  if(metric.type==='choice')return String(value);
...
export function createEncounterMetricEnvelope({object,event,metaRecord=null,recordedAt=null}={}){
  ...
  for(const metric of metricSchema.metrics.filter(metric=>metric.repeatability!=='per-entry')){
    envelope.values[metric.id]=valueForMetric(metric,event);
  }
