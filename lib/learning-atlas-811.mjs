...
const dialogueTail=(kind,track,seed)=>{
  ...
  let pool=functional;
  if(['schedule','prefer','decline','boundary'].includes(kind))pool=alignment;
  else if(['opinion','compare','cause','stance','story','feeling','reaction'].includes(kind))pool=reflective;
  else if(['smalltalk','conflictrepair'].includes(kind))pool=social;
  else if(track==='gym'||kind==='gymplan')pool=gym;
  else if(track==='work'||kind==='update'||kind==='feedback')pool=work;
  else if(track==='travel'||track==='service')pool=travel;
  ...
  for(const scene of AXIS811_SCENES){
    ...
    for(let li=0;li<AXIS811_LEVELS.length;li++){
      ...
      for(const [a,az] of scene.a)for(const [b,bz] of scene.b){
        ...
        const id=`en811${String(++seq).padStart(4,'0')}`,[turn5,turn6]=dialogueTail(scene.kind,scene.track,seq);
        units.push({
          id,lang:'en',target,en:target,zh:fill(zhTemplate,vars),
          scenario:scene.scenario,level,register:scene.register,nativeNote:note(level),alt:fill(altTemplate,vars),
          response,followup,closing,turn5,turn6,
          conversation:[target,response,followup,closing,turn5,turn6],
          pattern:compact(targetTemplate.replace(/\{a\}|\{b\}/g,'…').replace(/\{az\}|\{bz\}/g,'')),
...
export function auditAxis811Atlas(units=buildAxis811Atlas()){
  const required=['id','target','zh','scenario','level','register','response','followup','closing','turn5','turn6','connected','spelling','dictation','shadow','track','domain'];
  ...
  const sixTurn=units.every(x=>Array.isArray(x.conversation)&&x.conversation.length===6&&x.conversation.every(Boolean));
  return {count:units.length,duplicateTargets,levelCounts,trackCounts,missing,unresolved,fourTurn,sixTurn}
}