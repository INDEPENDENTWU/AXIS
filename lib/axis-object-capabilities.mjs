...
export const METRIC_CAPABILITIES=Object.freeze({
  ...
  reps:{id:'reps',label:'次数',group:'volume',type:'count',unit:'次',presentation:'stepper',min:0,step:1,repeatability:'per-entry',aggregation:'max',executionHint:'sets'},
...
export function resolveExecutionMode(metrics=[],requested='auto'){
  ...
  if(keys.has('sets')||keys.has('reps')||hints.has('sets'))return'sets';
