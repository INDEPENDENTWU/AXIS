...
function replaceStatementByPrefix(source,prefix,to,label){
 ...
 const start=hits[0],end=source.indexOf(';',start);if(end<0)fail(`${label} missing terminator`);
...
src=replaceStatementByPrefix(src,'window.__AXIS_MEDIA_STORE__=',"window.__AXIS_MEDIA_STORE__={get:getMedia,put:putMedia,del:deleteMedia,format:AXIS_MEDIA_FORMAT,entries:axisBackupMediaEntries,replaceAll:axisBackupMediaReplaceAll,keys:axisBackupMediaKeys};",'media transport bridge v2 keys');
src=replaceStatementByPrefix(src,'window.__AXIS_PORTABLE_BACKUP__=',"window.__AXIS_PORTABLE_BACKUP__={schema:AXIS_BACKUP_SCHEMA,exportSchema:AXIS_BACKUP_V2_SCHEMA,storagePrefix:AXIS_BACKUP_PREFIX,owner:'transport-only',network:false,create:axisBackupCreateBundle,verify:axisBackupVerifyBundle,createFile:axisBackupV2BuildFile,verifyFile:axisBackupV2VerifyFile,restore:async bundle=>{if(state.active)throw new Error('restore-active-session-blocked');const verified=await axisBackupVerifyBundle(bundle);verified.format='v1';window.__AXIS_BACKUP_PENDING__={bundle,verified};return axisBackupRestorePending()},restoreFile:async file=>{if(state.active)throw new Error('restore-active-session-blocked');const parsed=await axisBackupV2VerifyFile(file);window.__AXIS_BACKUP_PENDING__=parsed;return axisBackupRestorePending()}};",'portable backup v2 public transport API');
