...
const fail=m=>{throw new Error(`[AXIS 8.14 Evolution Objects release] ${m}`)};
...

{
 ...
 if(String(x.publicVersion)!=='8.13.1'||String(x.stableBaseVersion)!=='8.13.1')fail(`expected sealed 8.13.1 input, found ${x.publicVersion}/${x.stableBaseVersion}`);
 x.publicVersion='8.14';x.stableBaseVersion='8.14';write(f,JSON.stringify(x,null,2)+'\n');
}
{
 const f='prepare-882-version.mjs';let s=read(f);
 s=once(s,"const VERSION='8.13.1';","const VERSION='8.14';",'release version anchor');
 s=s.replaceAll('AXIS 8.13.1] release identity','AXIS 8.14] release identity');write(f,s);
}
...
for(const f of inherited){let s=read(f);const n=(s.match(/'8\.13\.1'/g)||[]).length;if(!n)fail(`${f} missing inherited 8.13.1 identity`);s=s.replaceAll("'8.13.1'","'8.14'");write(f,s)}
...
for(const f of ['scripts/axis-8124-flow-smoke.mjs','scripts/axis-8124-catalog-polish-smoke.mjs','scripts/axis-8124-custom-equipment-smoke.mjs','scripts/axis-8125-smart-create-polish-smoke.mjs']){
 let s=read(f);s=once(s,"assert.equal(await page.evaluate(()=>window.__AXIS_RELEASE__),'8.13.1');","assert.equal(await page.evaluate(()=>window.__AXIS_RELEASE__),'8.14');",`${f} current release assertion`);write(f,s);
}
{
 const f='scripts/axis-8131-evolution-smoke.mjs';let s=read(f);
 s=once(s,"assert.equal(await page.evaluate(()=>window.__AXIS_RELEASE__),'8.13.1');","assert.equal(await page.evaluate(()=>window.__AXIS_RELEASE__),'8.14');",'8.13.1 inherited smoke public identity');
...
}
{
 ...
 s=once(s,"if(contract.publicVersion!=='8.13.1'||contract.stableBaseVersion!=='8.13.1')","if(contract.publicVersion!=='8.14'||contract.stableBaseVersion!=='8.14')",'8.13.1 inherited postbuild release contract');
 s=once(s,"if(info.version!=='8.13.1'||info.baseVersion!=='8.13.1')","if(info.version!=='8.14'||info.baseVersion!=='8.14')",'8.13.1 inherited postbuild manifest contract');
 write(f,s);
}
