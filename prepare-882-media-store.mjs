...
function axisMediaRecordedUsage(){let photos=0,videos=0;for(const s of (state.active?[state.active]:[]).concat(state.sessions||[]))for(const e of ev(s)){photos+=(Number(e.photoBytes)||0)+(Number(e.sourcePhotoBytes)||0);videos+=(Number(e.videoBytes)||0)+(Number(e.sourceVideoBytes)||0)}return{photos,videos}}
async function axisMediaExactUsage(){const db=await openDb();return new Promise((res,rej)=>{let tx=null,req=null,settled=false,photos=0,videos=0;const bad=e=>{if(settled)return;settled=true;try{db.close()}catch{}rej(tx?.error||req?.error||e?.target?.error||e||new Error('media-usage-scan-failed'))};try{tx=db.transaction('media','readonly');req=tx.objectStore('media').openCursor();req.onsuccess=()=>{const cur=req.result;if(!cur){if(settled)return;settled=true;try{db.close()}catch{}res({photos,videos});return}const z=axisMediaStoredBytes(cur.value);if(/^(?:V-|SV-)/.test(String(cur.key)))videos+=z;else photos+=z;cur.continue()};req.onerror=bad;tx.onabort=bad;tx.onerror=()=>{}}catch(e){bad(e)}})}
...
if(!app.includes('Number(e.sourcePhotoBytes)')||!app.includes('Number(e.sourceVideoBytes)')||!app.includes('/^(?:V-|SV-)/'))fail('source-sidecar media accounting missing');
...
console.log('[AXIS 8.8.2 media store] PASS · app.js sole IndexedDB media owner · Blob compatibility read · ArrayBuffer structured-clone write · v61/v877/watermark delegate · boot storage brief metadata-only · exact storage scan cursor-bounded · source-sidecar aware');