...
/* 1. User-visible shell is final in source HTML, before any JavaScript hydration. */
let html=read('index.html');
html=regexOnce(html,
  /<button class="iconBtn" id="settingsBtn" aria-label="设置">[\s\S]*?<\/button>/,
  '<button class="iconBtn v877Control" id="settingsBtn" aria-label="AXIS 控制"><span class="v877ControlGlyph" aria-hidden="true"><i></i><i></i><b></b><b></b></span></button>',
  'static settings control');
html=regexOnce(html,
  /<div class="captureDock" id="dock">\s*<button class="scanPrimary" id="scanBtn">([\s\S]*?)<span>扫一下<\/span><\/button>\s*<\/div>/,
  '<div class="captureDock v8-dual" id="dock"><button class="scanPrimary" id="scanBtn">$1<span>拍摄记录</span></button><button id="quickRecordBtn" class="v8QuickBtn" type="button"><span>＋</span><b>快速记录</b></button></div>',
  'static dual recording dock');
...
/* 2. The pre-rendered quick-record button is wired by v61; v61 no longer creates top-level chrome. */
let v61=read('v61.js');
...
/* 3. Public release and stable fallback are separate concepts. */
...
/* 4. Optional 8.7.12 feature loading can degrade internally but never rewrites the public release backwards. */
...
'feature public-version owner');
feature=textOnce(feature,
`  window.__AXIS_VERSION__=BASE;\n  window.__AXIS_RELEASE__=BASE;\n  window.__AXIS_8712_READY__=false;\n  setVersionText(BASE);\n  readyRoot.dataset.axisRelease=BASE;`,
`  window.__AXIS_VERSION__=PUBLIC;\n  window.__AXIS_RELEASE__=PUBLIC;\n  window.__AXIS_8712_READY__=false;\n  setVersionText(PUBLIC);\n  readyRoot.dataset.axisRelease=PUBLIC;`,
'feature fallback public version');
write('postbuild-features-hardened.mjs',feature);

/* 5. Runtime polish may enhance features, but it cannot replace the settings-control DOM after first paint. */
...
/* 6. One canonical custom-equipment editor is shared by record and Settings. */
let app=read('app.js');
...
app=regexOnce(app,/\$\('#addCustomEq'\)\.onclick=\(\)=>\{closeSheet\('eqSheet'\);openCustomEditor\(\)\};/,"$('#addCustomEq').onclick=()=>{closeSheet('eqSheet');window.__AXIS_OPEN_CUSTOM_EQUIPMENT__()};",'record custom editor route');
app=regexOnce(app,/\$\('#newCustomEq'\)\.onclick=\(\)=>openCustomEditor\(\);/,"$('#newCustomEq').onclick=()=>window.__AXIS_OPEN_CUSTOM_EQUIPMENT__();",'settings custom editor route');
write('app.js',app);

/* 7. Nested sheet order is semantic, not DOM-order based. This makes Settings -> My equipment -> Custom editor visible. */
...
completion=textOnce(completion,
...
"  const xs=visibleSheets(),child=snap?.sheet?(xs.filter(x=>x!==snap.sheet).at(-1)||null):topSheet();\n  if(snap?.sheet&&child)addBack(child,snap.sheet,snap.scroll);\n  else if(child)frontSheet=child;\n  syncLayers();",
'wire newly opened child independent of DOM order');
