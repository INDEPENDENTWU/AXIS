...
async function axis89Decode(url){if(!url)return;try{const im=new Image();im.src=url;if(im.decode)await im.decode();else await new Promise(r=>{im.onload=im.onerror=r})}catch{}}
function axis89CommitDetail(txn,title,stage,urls,bind){
 ...
 requestAnimationFrame(()=>{
  ...
  window.__AXIS_89_DETAIL__={owner:'atomic-handoff',txn,committedAt:Date.now(),visible:true}
 })
}
async function openEvent(id){
 const e=allEvents().find(x=>x.id===id);if(!e)return;const txn=++axis89DetailTxn,eq=eventEq(e),urls=[];let media='',decode=[];
 if(e.clipRef){const v=await axis89MediaUrl(e.clipRef,urls),poster=e.frameRefs?.[0]?await axis89MediaUrl(e.frameRefs[0],urls):'';if(txn!==axis89DetailTxn){axis89Revoke(urls);return}if(poster)decode.push(axis89Decode(poster));if(v)media+=\`<div class="videoCard"><div class="mediaTop"><b>现场片段</b><span>\${e.videoWatermarked?'带水印':'原片'}</span></div><video class="detailVideo" src="\${v}" \${poster?\`poster="\${poster}"\`:''} controls playsinline muted></video><button class="mediaSave" id="saveDetailVideo">保存视频</button></div>\`}
 if(e.frameRefs?.length){const imgs=[];for(const r of e.frameRefs){const u=await axis89MediaUrl(r,urls);if(txn!==axis89DetailTxn){axis89Revoke(urls);return}if(u){decode.push(axis89Decode(u));imgs.push(\`<img src="\${u}" alt="现场照片">\`)}}media+=\`<div class="mediaTop photoTop"><b>现场照片</b><span>\${imgs.length}张 · \${state.prefs.watermark.photoMode==='raw'?'原片':'带水印'}</span></div><div class="detailFilm">\${imgs.join('')}</div>\`}
 await Promise.allSettled(decode);if(txn!==axis89DetailTxn){axis89Revoke(urls);return}
 ...
 const stage=D.createElement('div');stage.innerHTML=media+\`<div class="detailList">\${rows.map(r=>\`<div class="detailRow"><span>\${r[0]}</span><span>\${esc(r[1])}</span></div>\`).join('')}</div>\${e.frameRefs?.[0]?'<div class="detailActions"><button id="saveDetailPhoto">保存图片</button><button id="makeSessionReport">训练报告</button></div>':''}\`;
 ...
 axis89CommitDetail(txn,e.name,stage,urls,bind)
}
...
console.log('[AXIS 8.9 detail] PASS');
