...
function axis89CommitDetail(txn,title,stage,urls,bind){
 ...
 requestAnimationFrame(()=>{
  ...
  window.__AXIS_89_DETAIL__={owner:'atomic-handoff',txn,committedAt:Date.now(),visible:true,mediaDecodeBlocking:false}
 })
}
async function openEvent(id){
 const e=allEvents().find(x=>x.id===id);if(!e)return;const txn=++axis89DetailTxn,eq=eventEq(e),urls=[];let media='';
 if(e.clipRef){const v=await axis89MediaUrl(e.clipRef,urls),poster=e.frameRefs?.[0]?await axis89MediaUrl(e.frameRefs[0],urls):'';if(txn!==axis89DetailTxn){axis89Revoke(urls);return}if(v)media+=\`<div class="videoCard"><div class="mediaTop"><b>现场片段</b><span>\${e.videoWatermarked?'带水印':'原片'}</span></div><video class="detailVideo" src="\${v}" \${poster?\`poster="\${poster}"\`:''} controls playsinline muted></video><button class="mediaSave" id="saveDetailVideo">保存视频</button></div>\`}
...
function openSession`;
 ...
 if(!src.includes('mediaDecodeBlocking:false'))fail('non-blocking detail diagnostic missing');
 syntax(src,FILE);write(FILE,src);
}
console.log('[AXIS 8.9 detail] PASS · atomic content commit no longer waits on media decode');
