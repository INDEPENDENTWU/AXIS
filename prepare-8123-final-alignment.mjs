...
(function axis8123InstallFinalSettingsAlignment(){
 ...
 const s=D.createElement('style');s.id='v8123FinalSettingsAlignment';s.textContent=
  '#settingsSheet #v813LearningGate,#settingsSheet #v813ServiceGate{padding-left:0!important;padding-right:0!important}'+
  ...
  '@media(max-width:380px){#settingsSheet #v813LearningGate>.settingLink,#settingsSheet #v813ServiceGate>.settingLink{padding-left:24px!important;padding-right:24px!important}}';
 D.head.appendChild(s)
})();
try{window.__AXIS_8123_FINAL_ALIGNMENT__={version:'8.12.3',nativeRowInset:24,gateHorizontalInset:0,foldHorizontalInset:18,trainingOwner:false}}catch{}
`;

src=src.slice(0,end)+block+'\n'+src.slice(end);
for(const needle of ['__AXIS_8123_FINAL_ALIGNMENT__','nativeRowInset:24','gateHorizontalInset:0'])if(!src.includes(needle))fail(`missing ${needle}`);
try{new Function(src)}catch(e){fail(`runtime syntax ${e.message}`)}
fs.writeFileSync(FILE,src);
console.log('[AXIS 8.12.3 final alignment] PASS · Learning + Cloud/AI rows share native 24px inset · fold content preserved · no training ownership');
