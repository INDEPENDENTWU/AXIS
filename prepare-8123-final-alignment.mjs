...
(function axis8123InstallFinalSettingsAlignment(){
 ...
 const s=D.createElement('style');s.id='v8123FinalSettingsAlignment';s.textContent=
  ...
  '#settingsSheet #v813LearningGate>.settingLink,#settingsSheet #v813ServiceGate>.settingLink{margin:0!important;padding-left:0!important;padding-right:0!important;transform:none!important}'+
  ...
  '@media(max-width:380px){#settingsSheet #v813LearningGate>.settingLink,#settingsSheet #v813ServiceGate>.settingLink{margin:0!important;padding-left:0!important;padding-right:0!important}}';
 D.head.appendChild(s)
})();
try{window.__AXIS_8123_FINAL_ALIGNMENT__={version:'8.12.3',nativeRowLocalInset:0,legacyGateClassRetired:true,foldHorizontalInset:18,trainingOwner:false}}catch{}
`;

src=src.slice(0,end)+block+'\n'+src.slice(end);
for(const needle of ['__AXIS_8123_FINAL_ALIGNMENT__','nativeRowLocalInset:0','legacyGateClassRetired:true'])if(!src.includes(needle))fail(`missing ${needle}`);
try{new Function(src)}catch(e){fail(`runtime syntax ${e.message}`)}
fs.writeFileSync(FILE,src);
console.log('[AXIS 8.12.3 final alignment] PASS · legacy gate class retired before mount · native row local inset 0 · fold content preserved · no training ownership');
