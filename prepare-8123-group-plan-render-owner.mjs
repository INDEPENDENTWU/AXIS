...
const helper=`function axis8123EnsurePlanEntry(){
 ...
 const owned=window.__AXIS_RECORDING__?.first?.(),n=Math.max(1,Number(owned?.count)||$$('.v8SetRow',h).length||1);
...
try{window.__AXIS_8123_GROUP_PLAN_RENDER_OWNER__={version:'8.12.3',owner:'v61-renderSets',buttonClass:'v8121PlanButton',dataset:'v875Plan+v8123Plan',staleNode:false}}catch{}
`;
...
for(const needle of ['__AXIS_8123_GROUP_PLAN_RENDER_OWNER__',"owner:'v61-renderSets'","className='v875PlanEntry v8121PlanButton v8123PlanEntry'","dataset.v875Plan='1'","dataset.v8123Plan='1'",'axis8123EnsurePlanEntry();emitRecording'])if(!src.includes(needle))fail(`missing ${needle}`);
try{new Function(src)}catch(e){fail(`v61 syntax ${e.message}`)}
fs.writeFileSync(FILE,src);
console.log('[AXIS 8.12.3 Group Plan render owner] PASS · every canonical set render synchronously restores one native planner button before external enhancement callbacks');
