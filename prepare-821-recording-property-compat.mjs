...
// Earlier canonicalization may normalize $$ collection selectors to $. Do not
// depend on either helper shape here: resolve the existing metric inputs directly
// from the document collection, then find the one canonical value owner.
const metricLookupCandidates=[
 "function axis821MetricInput(key){return $$('[data-axis818-metric]').find(x=>x.dataset.axis818Metric===String(key))||null}",
 "function axis821MetricInput(key){return $('[data-axis818-metric]').find(x=>x.dataset.axis818Metric===String(key))||null}"
];
const metricLookupHits=metricLookupCandidates.reduce((n,x)=>n+(src.split(x).length-1),0);
if(metricLookupHits!==1)fail(`canonical metric input lookup expected once after selector normalization, found ${metricLookupHits}`);
const metricLookupFrom=metricLookupCandidates.find(x=>src.includes(x));
const metricLookupTo="function axis821MetricInput(key){return Array.from(D.querySelectorAll('[data-axis818-metric]')).find(x=>x.dataset.axis818Metric===String(key))||null}";
src=src.replace(metricLookupFrom,metricLookupTo);
if(!src.includes("Array.from(D.querySelectorAll('[data-axis818-metric]')).find"))fail('collection-safe metric input lookup missing');

try{new Function(src)}catch(e){fail(`app syntax ${e.message}`)}
fs.writeFileSync(FILE,src);
console.log('[AXIS 8.21 recording property compat] PASS · canonical recorder resolves Object Truth + preserves active-review visibility + keeps in-progress values + rating rail shares one directly editable value owner + metric actions survive selector normalization');
