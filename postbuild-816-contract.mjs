...
const compositor818=contract.publicVersion==='8.18';
if(compositor818){
...
info.axis816={release:true,scope:'capture-field-comparative-evidence',capture:{surface:'v816-capture-field',cameraOwner:'app.js',persistenceOwner:'app.js',mediaStore:'axis_v42_media',entryPoints:['capture-record','quick-record-supplement'],photo:{multi:true,maxPerEncounter:12,cover:'frameRefs[0]-reorder-no-schema'},video:{enabled:true,maxSeconds:60,maxPerEncounter:1,clipSchema:'clipRef',audio:false,autostop:true,bitrateTarget:2500000,laterCompositorSupersessionAllowed:true},scan:{preserved:true,usesExistingPreference:true,addsFramesToDraft:true},newStorage:false,newSchema:false},evidence:{owner:'v815-media-evidence',mode:'arbitrary-two-point',photoBearingEncounters:true,presets:['ends','recent','adjacent'],railSelectsActiveSide:true,stableInPlace:true,autoplay:false,factualOnly:true},ownership:{trainingState:false,newPersistence:false,newNetwork:false,newAi:false,newRecorderOwner:false,replay:false},next:{replay:'deferred-until-capture-density-and-selection-semantics-are-sealed'}};
fs.writeFileSync('axis-build.json',JSON.stringify(info,null,2)+'\n');
console.log(`[AXIS 8.16 contract] PASS · unified Capture Field · <=12 photos · one <=60s silent clip · ${compositor818?'8.18 stable 30fps compositor':'8.16 bounded direct recorder'} · canonical F/V pointers preserved with optional additive S/SV sources · existing app/IndexedDB ownership · arbitrary factual two-point evidence · Replay deferred`);
