...
/* 8.17 intentionally collapsed the historical three Quick Record media buttons
   ...
   useful by asserting the one current entry still delegates to the canonical Capture
   bridge for the selected custom equipment. */
...
assert.deepEqual(await page.evaluate(()=>window.__AXIS_882_MEDIA_CALLS__),[{mode:'photo',id:'custom-waist'}]);`;
...
if(!c.includes("Quick Record must expose one current supplemental Evidence entry")||!c.includes('8.8.2 inherited visual-memory capture draft cannot complete'))fail('8.8.2 inherited flow assertions missing');
fs.writeFileSync(smoke882,c);

console.log('[AXIS 8.18 inherited test-flow seal] PASS · 8.12.1 + 8.12.3 photo flows use canonical draft → 完成 → review · 8.8.2 Quick Evidence is one canonical handoff · visual-memory capture uses current completion flow');
